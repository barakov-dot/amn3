...
AMN2_SOURCE_ZIP="${AMN2_SOURCE_ZIP:-/root/amn2-private-telegram-smoke-overlay-801f8c3/amn2-codex-vps-test-prep-801f8c3-source.zip}"
AMN2_EXPECTED_SOURCE_SHA="${AMN2_EXPECTED_SOURCE_SHA:-B332CB1DCFB85768ACE0DF78E038B955F7C853989CF1C67CDE0233FA51EBD6C3}"
AMN2_EXPECTED_SOURCE_COMMIT="${AMN2_EXPECTED_SOURCE_COMMIT:-801f8c3}"
...
PIP_INSTALL_STATUS="passed"
if ! "$PYTHON_BIN" -m pip install -e . > "$RUN_ROOT/pip-install-editable.txt" 2>&1; then
  if grep -Fq 'Could not find a version that satisfies the requirement setuptools>=69' "$RUN_ROOT/pip-install-editable.txt" \
    && grep -Fq 'No matching distribution found for setuptools>=69' "$RUN_ROOT/pip-install-editable.txt"; then
    PIP_INSTALL_STATUS="skipped_existing_source_path_after_expected_offline_build_stop"
    log "editable install skipped after exact known offline build-isolation diagnostic"
  else
    die "editable install failed unexpectedly; see $RUN_ROOT/pip-install-editable.txt"
  fi
fi

"$PYTHON_BIN" - "$AMN2_DIR" <<'PY' > "$RUN_ROOT/source-import-check.txt"
import importlib
import sys
from pathlib import Path
...
expected = (Path(sys.argv[1]) / "app" / "__init__.py").resolve()
actual = Path(app_module.__file__).resolve()
if actual != expected:
    raise SystemExit(f"active source path mismatch: actual={actual} expected={expected}")
print(f"active_source_path={actual}")
PY
