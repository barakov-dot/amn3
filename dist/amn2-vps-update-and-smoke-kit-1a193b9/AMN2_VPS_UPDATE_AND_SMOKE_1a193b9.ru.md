# AMN2 VPS Update And Smoke Kit 1a193b9

Дата: 2026-06-06.

Назначение: обновить существующий `/opt/amn2` до `amn2/codex-vps-test-prep` head `1a193b9` и выполнить read-only/API/web-panel gate без live peer mutations.
...
```bash
cd /root
sha256sum -c amn2-vps-update-and-smoke-kit-1a193b9.zip.sha256.txt
mkdir -p amn2-vps-update-and-smoke-kit-1a193b9
python3 -m zipfile -e amn2-vps-update-and-smoke-kit-1a193b9.zip amn2-vps-update-and-smoke-kit-1a193b9
cd amn2-vps-update-and-smoke-kit-1a193b9
sha256sum -c amn2-codex-vps-test-prep-1a193b9-source.zip.sha256.txt
```

## 2. Применить source overlay

```bash
cd /root/amn2-vps-update-and-smoke-kit-1a193b9
...
Этот kit закрывает read-only/API/web-panel update+smoke package baseline для stable `1a193b9`. Он не запускает live peer apply/revoke и не открывает broad write/API/config/agent surfaces.
