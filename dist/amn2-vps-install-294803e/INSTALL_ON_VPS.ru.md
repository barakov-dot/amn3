...
```bash
sudo apt update
sudo apt install -y python3.12 python3.12-venv python3-pip passwd adduser
```
...
```bash
scp amn2-source-294803e.zip INSTALL_ON_VPS.ru.md install_on_vps.sh amn2_api_loopback_smoke.sh root@YOUR_VPS:/root/amn2-install/
```
...
Для обновления уже настроенного `/opt/amn2`, где нужно сохранить `.env`, `data/`,
`venv/` и `servers.yml`, безопаснее использовать:

```text
amn2-vps-update-and-smoke-kit-294803e.zip
```
...
apply только после read-only/dry-run проверок и отдельного подтверждения.
...
## 5. Проверить web-панель через loopback
...
```bash
ssh -L 3030:127.0.0.1:3030 root@YOUR_VPS_HOST
```
...
Минимум для API/web-panel slice: `/api-readiness`, `/api-tokens`, issue raw API token one-time, refresh without raw token/hash, revoke.

## 6. Запуск bot вручную

```bash
python -m app.main
```

## 7. Включить systemd только после ручной проверки
...
## 8. Что не делать первым шагом

- Не включать `VPS_APPLY_ENABLED=true` до dry-run/read-only gate.
- Не хранить raw web password, Telegram token, private key или PSK в shell history.
- Не публиковать web-панель `:3030` в интернет без firewall, reverse proxy, HTTPS или SSH tunnel.
- Не открывать external API bind без отдельного reverse proxy/TLS/rate-limit/auth decision.
- Не запускать `apply-peer --apply` или `revoke-peer --apply` без отдельного операторского подтверждения.
