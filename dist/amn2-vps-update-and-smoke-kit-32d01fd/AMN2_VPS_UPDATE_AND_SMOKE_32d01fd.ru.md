# AMN2 VPS Update And Smoke Kit 32d01fd

Дата: 2026-06-06.

Назначение: обновить существующий `/opt/amn2` до `amn2/codex-vps-test-prep` head `32d01fd` и выполнить read-only/API/web-panel gate без live peer mutations.

`32d01fd` обновляет только read-only integration status contract: Phase 2 live gate теперь отражается как `verified_live`, а следующий gate - operator-only controlled prod readiness. Этот kit не включает live apply/revoke и не открывает новые public/write/config/agent/backup поверхности.
...
sha256sum -c amn2-codex-vps-test-prep-32d01fd-source.zip.sha256.txt
```

## 2. Применить source overlay

```bash
cd /root/amn2-vps-update-and-smoke-kit-32d01fd
...
После read-only smoke pass `32d01fd` станет новым VPS-smoked runtime/source baseline для controlled prod readiness. Это не разрешает public prod, live peer mutations или broad API/config/agent/backup surfaces.
