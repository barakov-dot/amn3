# AMN2 VPS Update And Smoke Kit 7764ae7

Дата: 2026-06-04.

Назначение: обновить существующий `/opt/amn2` до `amn2/codex-vps-test-prep` head `7764ae7` и выполнить read-only/API/web-panel gate без live peer mutations.
...
sha256sum -c amn2-codex-vps-test-prep-7764ae7-source.zip.sha256.txt
```

## 2. Применить source overlay

```bash
cd /root/amn2-vps-update-and-smoke-kit-7764ae7
...
Этот kit закрывает Phase 1 read-only/API/web-panel baseline для `7764ae7`. Phase 2 live single test peer apply/revoke остается отдельным новым чатом/gate с отдельным подтверждением оператора и rollback checklist.
