"""Offline preview; one exact zero-input SSH diagnostic if approved."""
...
Stop = core.Stop
APPROVAL = "PHASE16_SSH_ZERO_INPUT_PREFLIGHT_20260922_005"
GATE_MANIFEST_RELATIVE = Path(
    "research/amn2/phase16-bot-transport-preflight-gate-005-manifest-2026-09-22.json"
)
EVIDENCE_DIRECTORY = Path(
    "C:/Users/SooL/Documents/VPS-OPS-LAB/worktrees/"
    "phase16-bot-integration-readback-runner-20260922/execution-005"
)
...
def pass_receipt():
    return {
        "schema": "phase16.ssh-zero-input-preflight-remote.v1",
        "status": "SSH_ZERO_INPUT_PREFLIGHT_PASS",
        "approval": APPROVAL,
        "input_bytes": 0,
        "production_reads": 0,
        "service_actions": 0,
...
def remote_command():
    encoded = json.dumps(pass_receipt(), sort_keys=True, separators=(",", ":"))
    code = (
        "import sys;"
...
def validate_receipt(value, returncode):
    core.require(returncode == 0 and value == pass_receipt(), "receipt_binding")
    return value
...
def validate_gate_manifest(value, root=ROOT):
    ...
    core.require(
        ...
        value.get("schema") == "phase16.ssh-zero-input-preflight-gate-manifest.v1" and
        value.get("status") == "ZERO_INPUT_PREFLIGHT_READY_NOT_EXECUTED" and
        value.get("amn3_base") == "badc14d9f7d5" and
    ...
    blobs = {
        "local_runner": canonical(root / "scripts/phase16_bot_transport_preflight_gate.py"),
    ...
    core.require(value.get("command_contract") == {
        "entrypoint": "scripts/phase16_bot_transport_preflight_gate.py",
...
def execute_once(evidence_dir, *, approval, approved_gate_sha, manifest, loader,
    ...
    claim = {
        "schema": "phase16.ssh-zero-input-preflight-claim.v1",
        "scope": APPROVAL,
        "attempts": 1,
    ...
    result = {
        "schema": "phase16.ssh-zero-input-preflight-local.v1",
        "claim": claim,
    ...
    try:
        ...
        returncode, output = transport(
            args, cwd=evidence_dir, env=environment, timeout=20, cap=4096,
            input_bytes=b"", diagnostics=result["transport"],
        )
...
def main(argv=None):
    ...
    try:
        ...
        preview = {
            "status": "ZERO_INPUT_PREFLIGHT_READY_NOT_EXECUTED",
        ...
        return 0 if result["status"] == "SSH_ZERO_INPUT_PREFLIGHT_PASS" else 3
    except Exception as error:
        reason = str(error)
