...
$expectedDockerTreeSha = "9aaf13904fd0738d88fe13db54527f6426f783b06664e3baf6e41ff140755aee"
...
if ($remoteExecutor.ExitCode -ne 0 -or $remoteExecutorText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  $([regex]::Escape($remoteExecutorPath))$" -or $remoteExecutorText -notmatch "(?m)^$expectedExecutorBytes$") { throw "Remote transaction 52FAB7 terminal recovery executor checksum/size mismatch." }
...
if ($precondition.ExitCode -ne 0 -or $preconditionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $preconditionText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "Transaction 52FAB7 terminal recovery precondition checksum mismatch." }
...
if ($result.ExitCode -ne 0) {
    ...
    throw "Transaction 52FAB7 terminal recovery failed; current transaction remains fail-closed."
}
