...
$expectedNonce = "8c0eb7e9b1345f599c33de55f9b00779b553aa7b5d570b41d54da497074310c3"
$expectedTransactionSha = "7f0684a098ee51ec21d01f0240f91ed0e49036b9c9784028ce8212f615df016c"
$expectedCapsuleSha = "38a07bbb54172f677883043b3c2280999ebb0305654a2fef57850576c4cc710a"
$expectedDockerTreeSha = "92f854a48f607bb29eca714bd42cceca0ca7bf14ab3533bf26596970edd29cbb"
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$" -or $remoteText -notmatch "(?m)^$expectedExecutorBytes$") { throw "Remote 8C0EB7 bundled recovery executor checksum mismatch." }
...
if ($remoteTransaction.ExitCode -ne 0 -or $remoteTransactionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $remoteTransactionText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "8C0EB7 bundled recovery transaction checksum mismatch." }
...
if ($cleanupResult.ExitCode -ne 0) {
    ...
    throw "8C0EB7 manual cleanup failed; retained package tree was not accepted as safely removable."
}
$packageAbsent = Invoke-ExactSsh (@($sshBase + @($target, "test ! -e /opt/amn2-spain-package && test ! -L /opt/amn2-spain-package"))) ([byte[]]@())
if ($packageAbsent.ExitCode -ne 0) { throw "8C0EB7 manual cleanup package absence verification failed." }
...
if ($terminalResult.ExitCode -ne 0) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($safeRemoteError)) { [Console]::Error.WriteLine($safeRemoteError) }
    throw "Transaction 8C0EB7 terminal recovery failed; current transaction remains fail-closed."
}
