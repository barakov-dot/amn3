...
main() {
    ...
    for required in ss ip docker systemctl sha256sum cut sort date; do
        if ! require_command "${required}"; then
            emit_failure bootstrap 69
            return 69
        fi
    done
    ...
    if ! docker_names=$(docker ps -a --format '{{.Names}}'); then
        emit_failure "${stage}" 72
        return 72
    fi
    set +e
    ...
    if ! docker_network_ids=$(docker network ls -q); then
        emit_failure "${stage}" 72
        return 72
    fi
    docker_subnets=''
    while IFS= read -r docker_network_id; do
        ...
        if ! classification=$(docker network inspect --format '{{range .IPAM.Config}}{{println .Subnet}}{{end}}' "${docker_network_id}"); then
            emit_failure "${stage}" 72
            return 72
        fi
    ...
    observed_persistent=${AMN2_PHASE13_AWG2_PERSISTENT_PEERS-}
    observed_live=${AMN2_PHASE13_AWG2_LIVE_PEERS-}
    observed_restart=${AMN2_PHASE13_AWG2_RESTART_COUNT-}
    observed_forward=${AMN2_PHASE13_AWG2_FORWARD_RULE_COUNT-}
    observed_web=${AMN2_PHASE13_AWG2_WEB_LISTENER_EQUAL-}
    observed_bot=${AMN2_PHASE13_AWG2_BOT_DISABLED-}
    peer_set_sha256=${AMN2_PHASE13_AWG2_PEER_SET_SHA256-}
    if ! is_sha256 "${peer_set_sha256}"; then
        emit_failure "${stage}" 72
        return 72
    fi
    set +e
    classification=$(validate_awg2_projection \
        "${AMN2_PHASE13_AWG2_UDP_PORT-}" "${AMN2_PHASE13_AWG2_VPN_CIDR-}" \
        "${AMN2_PHASE13_AWG2_BRIDGE-}" "${observed_persistent}" "${observed_live}" \
        "${observed_restart}" "${observed_forward}" "${observed_web}" "${observed_bot}" \
        "${peer_set_sha256}" "${AMN2_PHASE13_AWG2_LIVE_PEER_SET_SHA256-}")
    rc=$?
    set -e
    if (( rc != 0 )); then
        emit_failure "${stage}" "${rc}"
    ...
    observed_foreign_entries=${AMN2_PHASE13_FOREIGN_PERSISTENT_ENTRIES-}
    observed_foreign_sha=${AMN2_PHASE13_FOREIGN_STABLE_SHA256-}
    observed_foreign_changed=${AMN2_PHASE13_FOREIGN_CHANGED-}
    observed_foreign_equal=${AMN2_PHASE13_FOREIGN_EQUAL-}
    set +e
    classification=$(validate_foreign_projection \
        ...
        "${AMN2_PHASE13_FOREIGN_RECEIPT_SHA256-}")
    rc=$?
    set -e
    if (( rc != 0 )); then
        emit_failure "${stage}" "${rc}"
