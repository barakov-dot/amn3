...
MANIFEST_SHA256 = "dc8462f415890d1d8bb975f915531a314fe8bfc85a5549e1f85b58d81c3228dc"
PAYLOAD_SIZE = 46408
PAYLOAD_SHA256 = "0d04ec697a808dffb02c17380d5b23a6be862e9350a5f0e5533f23e49a01ed05"
...
def parse_payload(payload):
    ...
    require(isinstance(manifest, dict) and manifest.get("schema") ==
            "phase16.integration-manifest.v1" and len(manifest.get("source", {})) == 126 and
...
if len(p)!=46408 or hashlib.sha256(p).hexdigest()!='0d04ec697a808dffb02c17380d5b23a6be862e9350a5f0e5533f23e49a01ed05' or p[:8]!=b'P16IRB01': stop('payload_binding')
c=int(p[8:16]);m=int(p[16:24])
if (c,m)!=(23282,23102): stop('payload_binding')
core=p[24:24+c]; manifest=p[24+c:]
if hashlib.sha256(core).hexdigest()!='f6b152a3439a3c89d6ea900133df4e6806ca0cc01e0bb893c22d59f82b5adaee' or hashlib.sha256(manifest).hexdigest()!='dc8462f415890d1d8bb975f915531a314fe8bfc85a5549e1f85b58d81c3228dc': stop('payload_binding')
...
def collect_database(payload):
    ...
    require(returncode == 0 and process["stderr_bytes"] == 0, "database_child")
    try:
...
def execute(payload):
    ...
    host = host_receipt(core)
    PARTIAL["host"] = host
    units_before = collect_units(core)
    PARTIAL["units_before"] = units_before
    source = stable_collect(core, SOURCE_ROOT, core.collect_source, manifest["source"])
    PARTIAL["source"] = source
    dependencies = stable_collect(core, DEPENDENCY_ROOT, core.collect_dependencies,
                                  manifest["runtime_pins"])
    PARTIAL["dependencies"] = dependencies
    files_before, identities = database_files(core)
    PARTIAL["database_files_before"] = files_before
    database, child = collect_database(payload)
    PARTIAL["database"] = database
    PARTIAL["database_child"] = child
    holders = core.collect_holders("/proc", identities,
    ...
    files_after, _ = database_files(core)
    sizes_stable = stable_roots(files_before, files_after)
    units_after = collect_units(core)
...
def stop_receipt(reason, partial=None):
    ...
    return {"schema": "phase16.integration-readback-remote.v1", "status": "STOP_NO_RETRY",
