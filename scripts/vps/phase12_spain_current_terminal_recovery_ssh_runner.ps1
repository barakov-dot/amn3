...
$expectedCapsuleSha = "2e146365a29c89e9466a8e54e174a3d4d2c969b2bfaeedc9531a59ec4f756a18"
$expectedDockerTreeSha = "6051924206a20bab41384c9def68cb7d09ab02756515a0dcc05c7e290e3f3248"
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$") { throw "Remote current terminal recovery executor checksum mismatch." }
...
if ($remoteRecovery.ExitCode -ne 0 -or $remoteRecoveryText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $remoteRecoveryText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "Current terminal recovery precondition checksum mismatch." }
...
if ($result.ExitCode -ne 0) {
    ...
    throw "Current terminal recovery failed; current transaction remains fail-closed."
}
