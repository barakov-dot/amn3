    ...
    with os.scandir(root) as iterator:
        for entry in iterator:
            ...
            if entry.name.endswith('.dist-info'):
                ...
                data = safe_read(Path(entry.path)/'METADATA', 65536)
                total += len(data)
...
def collect_schema(conn, allowed, *, deadline=None):
    ...
    try:
        ...
        for table in sorted(names):
            ...
            normalized = []
            for _, name, datatype, notnull, _default, pk, hidden in columns:
                require(name in tables[table] and datatype.upper() in ('INTEGER','INT','TEXT','REAL','BLOB','NUMERIC',''), 'schema_unknown')
                normalized.append([name,datatype.upper(),notnull,pk,hidden])
            ...
            for _, name, unique, origin, partial in query('PRAGMA index_list("'+table+'")'):
                ...
                index_columns = [r[2] for r in query('PRAGMA index_info("'+name+'")')]
                require(all(c in tables[table] for c in index_columns), 'schema_expression_index')
                item['indexes'].append({'name':name,'unique':unique,'columns':index_columns,'partial':partial})
        ...
        return {'status':'SHAPE_ONLY','sqlite_version':sqlite3.sqlite_version,
