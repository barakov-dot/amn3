...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$") { throw "Remote transaction 00D9DA manual cleanup executor checksum mismatch." }
...
if ($remoteTransaction.ExitCode -ne 0 -or $remoteTransactionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$") { throw "Transaction 00D9DA manual cleanup transaction checksum mismatch." }
...
if ($result.ExitCode -ne 0) {
    ...
    throw "Transaction 00D9DA manual cleanup failed; retained package tree was not accepted as safely removable."
}
