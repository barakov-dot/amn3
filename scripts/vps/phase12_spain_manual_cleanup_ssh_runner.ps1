...
$expectedExecutorSha = "0E736B9DDF950DA050FE945F7D5F6D860F9C782A45066AE42429CCF56EF05585"
$expectedExecutorBytes = 140277
...
$artifactRoot = Join-Path $repoRoot "private-artifacts\phase12-spain-manual-cleanup-v1-20260723"
$executorPath = Join-Path $artifactRoot "amn2-spain-phase12-manual-cleanup-executor.pyz"
...
$runnerApproval = "APPROVE PHASE12 SPAIN MANUAL RECOVERY CLEANUP RUNNER SHA256 $expectedRunnerSha EXECUTOR SHA256 $expectedExecutorSha EXECUTOR BYTES $expectedExecutorBytes NONCE $expectedNonce REMOVE ONLY VERIFIED RETAINED PACKAGE TREE /opt/amn2-spain-package PRESERVE TERMINAL LEDGER NO AMN2 START NO FOREIGN SERVICE MUTATION USA ROLLBACK CONTOUR"
if ($Approval -cne $runnerApproval) { Write-Output $runnerApproval; throw "Exact manual cleanup approval mismatch." }
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$") {
    & $scpExe @scpBase $executorPath "${target}:/root/amn2-spain-phase12-manual-cleanup-executor.pyz"
    if ($LASTEXITCODE -ne 0) { throw "Approved cleanup executor upload failed." }
    $uploadedHash = Invoke-ExactSsh (@($sshBase + @($target, "sha256sum /root/amn2-spain-phase12-manual-cleanup-executor.pyz"))) ([byte[]]@())
    $uploadedText = (New-Object Text.UTF8Encoding($false,$true)).GetString($uploadedHash.Stdout)
    if ($uploadedHash.ExitCode -ne 0 -or $uploadedText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-manual-cleanup-executor\.pyz$") { throw "Remote cleanup executor checksum mismatch." }
    $activation = Invoke-ExactSsh (@($sshBase + @($target, "mv -f /root/amn2-spain-phase12-manual-cleanup-executor.pyz /root/amn2-spain-phase12-executor.pyz"))) ([byte[]]@())
    if ($activation.ExitCode -ne 0) { throw "Remote cleanup executor activation failed." }
}
...
if ($result.ExitCode -ne 0) {
    ...
    throw "Manual cleanup failed; retained package tree was not accepted as safely removable."
}
