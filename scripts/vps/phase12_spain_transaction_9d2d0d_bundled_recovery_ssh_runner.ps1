...
$expectedNonce = "9d2d0d0b3bae9df38a717efd8995e4715893ec6ff861eb4824510ed9aa201889"
$expectedTransactionSha = "75f0f66c438d4115c0bf7640bdfbfec1c320ac8f06aac36229ad7681de0f1df2"
$expectedCapsuleSha = "9d912bd1d9bed6dd1c2678e6b4a8ee7b26d74ed9592b4b405ded82184427fd7c"
$expectedDockerTreeSha = "db76eb0118231f1f2aab6cb679e77453293bcbdd2157e5be3df7a2f644731013"
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$" -or $remoteText -notmatch "(?m)^$expectedExecutorBytes$") { throw "Remote 9D2D0D bundled recovery executor checksum mismatch." }
...
if ($remoteTransaction.ExitCode -ne 0 -or $remoteTransactionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $remoteTransactionText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "9D2D0D bundled recovery transaction checksum mismatch." }
...
if ($cleanupResult.ExitCode -ne 0) {
    ...
    throw "9D2D0D manual cleanup failed; retained package tree was not accepted as safely removable."
}
$packageAbsent = Invoke-ExactSsh (@($sshBase + @($target, "test ! -e /opt/amn2-spain-package && test ! -L /opt/amn2-spain-package"))) ([byte[]]@())
if ($packageAbsent.ExitCode -ne 0) { throw "9D2D0D manual cleanup package absence verification failed." }
...
if ($terminalResult.ExitCode -ne 0) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($safeRemoteError)) { [Console]::Error.WriteLine($safeRemoteError) }
    throw "Transaction 9D2D0D terminal recovery failed; current transaction remains fail-closed."
}
