...
$expectedNonce = "84aa1f36cdda4af588640e527c690a98dc364091591409ab0cf03ed21519ac59"
$expectedTransactionSha = "663d27a5bc97fcef339aef157c118194c688ed44d9913326e6c40728c3759238"
$expectedCapsuleSha = "b57058e9a374cebec4155a1797e316b021aa9113df262b2d90be903dca1134a2"
$expectedDockerTreeSha = "2affb8479f3fb5a465e1af48c9f30d9837e42151805f078384d443f893b4bbb7"
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$" -or $remoteText -notmatch "(?m)^$expectedExecutorBytes$") { throw "Remote 84AA1F bundled recovery executor checksum mismatch." }
...
if ($remoteTransaction.ExitCode -ne 0 -or $remoteTransactionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $remoteTransactionText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "84AA1F bundled recovery transaction checksum mismatch." }
...
if ($cleanupResult.ExitCode -ne 0) {
    ...
    throw "84AA1F manual cleanup failed; retained package tree was not accepted as safely removable."
}
$packageAbsent = Invoke-ExactSsh (@($sshBase + @($target, "test ! -e /opt/amn2-spain-package && test ! -L /opt/amn2-spain-package"))) ([byte[]]@())
if ($packageAbsent.ExitCode -ne 0) { throw "84AA1F manual cleanup package absence verification failed." }
...
if ($terminalResult.ExitCode -ne 0) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($safeRemoteError)) { [Console]::Error.WriteLine($safeRemoteError) }
    throw "Transaction 84AA1F terminal recovery failed; current transaction remains fail-closed."
}
