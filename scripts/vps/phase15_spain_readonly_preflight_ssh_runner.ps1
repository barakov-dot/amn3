...
function Get-Phase15AuthorizationInstant {
    $now = if ($null -eq $script:Phase15AuthorizationClock) { [DateTimeOffset]::UtcNow } else { & $script:Phase15AuthorizationClock }
...
function Test-Phase15ExactNonterminalTransactionJournal {
    param(
    ...
    try {
        ...
        if ($null -eq $Journal -or -not (Test-Phase15ExactProperties -Value $Journal -Required $required)) { return $false }
        ...
        $expectedTransactionPath = Get-Phase15TransactionPath -StateRoot $stateRoot -ClaimId $ClaimId
        $expectedOutcomeFullPath = [IO.Path]::GetFullPath($ExpectedOutcomePath)
        $expectedStagedPath = $expectedOutcomeFullPath + '.phase15-' + $ClaimId + '.staged'
        return (
            [IO.Path]::GetFullPath($TransactionPath) -ceq [IO.Path]::GetFullPath($expectedTransactionPath) -and
            $Journal.schema -ceq 'amn2.phase15.readonly-preflight-transaction.v2' -and
            ...
            $Journal.expected_host -ceq $ExpectedHost -and (Test-Phase15ExpectedHost -ExpectedHost $Journal.expected_host) -and
            $Journal.outcome_path -ceq $expectedOutcomeFullPath -and $Journal.staged_path -ceq $expectedStagedPath -and
            $Journal.reserved_at -ceq $ReservedAt -and (Test-Phase15UtcTimestamp -Value $Journal.reserved_at) -and
            (Test-Phase15UtcTimestamp -Value $Journal.started_at) -and
...
function Reset-Phase15UnstartedTransaction {
    param(
        [Parameter(Mandatory)][string]$TransactionPath,
        [Parameter(Mandatory)][string]$ClaimId,
        [Parameter(Mandatory)][string]$ManifestSha256,
    ...
    if (-not (Test-Phase15ExactNonterminalTransactionJournal -Journal $journal -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $ExpectedOutcomePath -ReservedAt $ReservedAt -ExpectedPhase 'transport_attempted' -ExpectedSshUsed $true)) { throw 'transaction_invalid' }
    $journal.phase = 'owned'
...
function Test-Phase15TransactionRequiresConservativeSshUsed {
    ...
    try {
        ...
        $journal = ConvertFrom-Phase15CanonicalJsonFile -Path $TransactionPath
        return -not (Test-Phase15ExactNonterminalTransactionJournal -Journal $journal -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $ExpectedOutcomePath -ReservedAt $ReservedAt -ExpectedPhase 'owned' -ExpectedSshUsed $false)
    } catch {
        return $true
    }
}
...
function Start-Phase15AuthorizedSshProcess {
    ...
    [void](Set-Phase15TransactionPhase -TransactionPath $TransactionPath -ClaimId $ClaimId -Phase 'transport_attempted' -Lock $Lock)
    $launchNow = Get-Phase15AuthorizationInstant
    $launchAt = $launchNow.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
    if (-not (Test-Phase15FutureClaim -Claim $Claim -ExpectedPackageId $script:Phase15PackageId -ExpectedManifestSha256 $ManifestSha256 -ExpectedCollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -At $launchAt)) {
        [void](Reset-Phase15UnstartedTransaction -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $ExpectedOutcomePath -ReservedAt $ReservedAt -Lock $Lock)
        throw 'claim_invalid'
    }
...
function Invoke-Phase15OneSshTransport {
    ...
    $cancellation = $null
    try {
        if (-not (Start-Phase15AuthorizedSshProcess -Process $process -Claim $Claim -ExpectedHost $ExpectedHost -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedOutcomePath $ExpectedOutcomePath -ReservedAt $ReservedAt -TransactionPath $TransactionPath -Lock $Lock)) { throw 'transport_failed' }
        $Started.Value = $true
...
function Get-Phase15OwnedStateResiduePaths {
    param(
...
function Assert-Phase15OwnedStateResiduesAbsent {
    param(
        [Parameter(Mandatory)][string]$LifecyclePath,
        [Parameter(Mandatory)][string]$OutcomePath,
        [string]$RecoveryOutcomePath,
    ...
    if (@(Get-Phase15OwnedStateResiduePaths -LifecyclePath $LifecyclePath -OutcomePath $OutcomePath -RecoveryOutcomePath $RecoveryOutcomePath -ClaimId $ClaimId).Count -ne 0) {
...
function Remove-Phase15OwnedStateResidues {
    ...
    foreach ($candidate in @(Get-Phase15OwnedStateResiduePaths -LifecyclePath $LifecyclePath -OutcomePath $OutcomePath -RecoveryOutcomePath $RecoveryOutcomePath -ClaimId $ClaimId)) {
        [IO.File]::Delete($candidate)
    }
    Assert-Phase15OwnedStateResiduesAbsent -LifecyclePath $LifecyclePath -OutcomePath $OutcomePath -RecoveryOutcomePath $RecoveryOutcomePath -ClaimId $ClaimId
}
...
function Test-Phase15TrustedOutcomeParentFacts {
    param(
    ...
    try {
        ...
        return Test-Phase15ManagedStateDirectoryFacts -Facts $Facts -ExpectedPath $expectedParent -AuthorizedSid $AuthorizedSid
    } catch {
        return $false
    }
}

function Assert-Phase15TrustedOutcomeParent {
    param(
        [Parameter(Mandatory)][string]$StateRoot,
        [Parameter(Mandatory)][string]$OutcomePath,
        [Parameter(Mandatory)][string]$AuthorizedSid
    ...
    $facts = Get-Phase15StateDirectoryFacts -Path $parent
    if (-not (Test-Phase15TrustedOutcomeParentFacts -Facts $facts -StateRoot $StateRoot -OutcomePath $fullOutcome -AuthorizedSid $AuthorizedSid)) { throw 'outcome_parent_invalid' }
    return $fullOutcome
}
...
function Start-Phase15Transaction {
    ...
    if ($ManifestSha256 -cnotmatch '^[0-9a-f]{64}$' -or $CollectorSha256 -cnotmatch '^[0-9a-f]{64}$' -or -not (Test-Phase15ExpectedHost -ExpectedHost $ExpectedHost) -or
        -not (Test-Phase15UtcTimestamp -Value $StartedAt) -or -not (Test-Phase15UtcTimestamp -Value $ReservedAt)) { throw 'transaction_invalid' }
    if (-not (Test-Phase15ClaimLock -Lock $Lock -StateRoot $StateRoot -ClaimId $ClaimId)) { throw 'claim_lock_invalid' }
    ...
    if (-not [string]::IsNullOrWhiteSpace($AuthorizedSid)) { $reservationPath = Assert-Phase15TrustedOutcomeParent -StateRoot $StateRoot -OutcomePath $reservationPath -AuthorizedSid $AuthorizedSid }
    ...
    try {
        ...
        $journal = [ordered]@{
            ...
            schema = 'amn2.phase15.readonly-preflight-transaction.v2'
            ssh_used = $false
        ...
        if (-not [string]::IsNullOrWhiteSpace($AuthorizedSid)) { [void](Assert-Phase15TrustedOutcomeParent -StateRoot $StateRoot -OutcomePath $reservationPath -AuthorizedSid $AuthorizedSid) }
...
function Test-Phase15ExactPublishedTerminalOutcome {
    ...
    try {
        ...
        if ($Document.manifest_sha256 -cne $Journal.manifest_sha256 -or $Document.collector_sha256 -cne $Journal.collector_sha256 -or
            ...
            $Document.started_at -cne $Journal.started_at -or -not (Test-Phase15UtcTimestamp -Value $Document.started_at) -or
            -not (Test-Phase15UtcTimestamp -Value $Document.ended_at)) { return $false }
...
function Reconcile-Phase15Transaction {
    ...
    if (-not (Test-Path -LiteralPath $journalPath -PathType Leaf)) {
        if ($hasExpectedBindings) { Remove-Phase15TransactionTemps -TransactionPath $journalPath -ClaimId $ClaimId }
    ...
    if ($hasExpectedBindings) { [void](Assert-Phase15TrustedOutcomeParent -StateRoot $StateRoot -OutcomePath $journal.outcome_path -AuthorizedSid $AuthorizedSid) }
    ...
    if ($null -ne $journal.terminal_path) {
        ...
        $staged = if ($stagedExists) { ConvertFrom-Phase15CanonicalJsonFile -Path $journal.staged_path } else { $null }
        if ($stagedExists -and $null -eq $staged) { throw 'transaction_invalid' }
        $stagedDigest = if ($null -ne $staged) { Get-Phase15CanonicalJsonSha256 -Value $staged } else { '' }
        $stagedValid = $null -ne $staged -and $stagedDigest -ceq $journal.terminal_outcome_sha256 -and (Test-Phase15ExactPublishedTerminalOutcome -Document $staged -Journal $journal)
        $publishedExists = Test-Path -LiteralPath $terminalPath -PathType Leaf
        $published = if ($publishedExists) { ConvertFrom-Phase15CanonicalJsonFile -Path $terminalPath } else { $null }
        ...
        $publishedValid = $null -ne $published -and $publishedDigest -ceq $journal.terminal_outcome_sha256 -and (Test-Phase15ExactPublishedTerminalOutcome -Document $published -Journal $journal)
        $lifecycleReserved = $null -ne $reservedLifecycle -and (Test-Phase15ExactProperties -Value $reservedLifecycle -Required @('claim_id','reason_code','reserved_at','status')) -and
        ...
        $lifecycleTerminal = $null -ne $reservedLifecycle -and (Test-Phase15ExactProperties -Value $reservedLifecycle -Required @('claim_id','ended_at','reason_code','status')) -and
        ...
        $outcomeOwned = Test-Phase15OutcomeOwnership -ReservationPath $journal.outcome_path -ClaimId $ClaimId
        ...
        if ($publishedValid) {
            ...
            Assert-Phase15OwnedStateResiduesAbsent -LifecyclePath $lifecyclePath -OutcomePath $journal.outcome_path -RecoveryOutcomePath $recoveryOutcomePath -ClaimId $ClaimId
        ...
        if ($terminalIsCurrent -and $journal.phase -ceq 'finalizing' -and -not $stagedExists -and $outcomeOwned) {
            $expectedCurrent = New-Phase15FailureOutcome -ReasonCode 'transport_failed' -ManifestSha256 $journal.manifest_sha256 -CollectorSha256 $journal.collector_sha256 -ExpectedHost $journal.expected_host -StartedAt $journal.started_at -EndedAt $journal.terminal_ended_at -SshUsed $journal.ssh_used
        ...
        if ($terminalIsRecovery -and $journal.phase -ceq 'finalizing' -and -not $stagedExists) {
            $expectedRecovery = New-Phase15FailureOutcome -ReasonCode 'transport_failed' -ManifestSha256 $journal.manifest_sha256 -CollectorSha256 $journal.collector_sha256 -ExpectedHost $journal.expected_host -StartedAt $journal.started_at -EndedAt $journal.terminal_ended_at -SshUsed $journal.ssh_used
    ...
    $failure = New-Phase15FailureOutcome -ReasonCode 'transport_failed' -ManifestSha256 $journal.manifest_sha256 -CollectorSha256 $journal.collector_sha256 -ExpectedHost $journal.expected_host -StartedAt $journal.started_at -EndedAt $EndedAt -SshUsed $sshUsed
...
function Publish-Phase15TerminalOutcome {
    ...
    if (-not [string]::IsNullOrWhiteSpace($TransactionPath)) {
        ...
        $isExactPublishableJournal = {
            ...
            if ($Status -ceq 'failed' -and $ReasonCode -ceq 'claim_invalid' -and (Test-Phase15ExactNonterminalTransactionJournal -Journal $Candidate -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $OutcomePath -ReservedAt $ReservedAt -ExpectedPhase 'owned' -ExpectedSshUsed $false)) { return $true }
            if ($Status -ceq 'failed' -and $ReasonCode -ceq 'transport_failed' -and (Test-Phase15ExactNonterminalTransactionJournal -Journal $Candidate -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $OutcomePath -ReservedAt $ReservedAt -ExpectedPhase 'transport_attempted' -ExpectedSshUsed $true)) { return $true }
            return Test-Phase15ExactNonterminalTransactionJournal -Journal $Candidate -TransactionPath $TransactionPath -ClaimId $ClaimId -ManifestSha256 $ManifestSha256 -CollectorSha256 $CollectorSha256 -ExpectedHost $ExpectedHost -ExpectedOutcomePath $OutcomePath -ReservedAt $ReservedAt -ExpectedPhase 'ssh_started' -ExpectedSshUsed $true
        }
        ...
        if ($Outcome.started_at -isnot [string] -or $Outcome.started_at -cne $journal.started_at -or -not (Test-Phase15UtcTimestamp -Value $Outcome.started_at)) { throw 'transaction_invalid' }
        $journal.terminal_ended_at = $EndedAt
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($TransactionPath)) {
            [void](Assert-Phase15TrustedManagedStateChain -StateRoot $transactionStateRoot -AuthorizedSid $AuthorizedSid -RequiredChildren @('claims','transactions','outcome-locks','outcomes'))
            $recoveryOutcomePath = Get-Phase15LifecyclePath -LifecycleRoot (Join-Path $transactionStateRoot 'recovery-outcomes') -ClaimId $ClaimId
            Remove-Phase15OwnedStateResidues -LifecyclePath $LifecyclePath -OutcomePath $OutcomePath -RecoveryOutcomePath $recoveryOutcomePath -ClaimId $ClaimId
            Assert-Phase15OwnedStateResiduesAbsent -LifecyclePath $LifecyclePath -OutcomePath $OutcomePath -RecoveryOutcomePath $recoveryOutcomePath -ClaimId $ClaimId
...
function Invoke-Phase15RunnerMain {
    ...
    $preProvisionNow = Get-Phase15AuthorizationInstant
    ...
    try {
        ...
        if (Test-Path -LiteralPath $lifecyclePath) { throw 'claim_replay' }
        $reservationNow = Get-Phase15AuthorizationInstant
        ...
        $transaction = Start-Phase15Transaction -StateRoot $stateRoot -OutcomePath $OutcomePath -ClaimId $claim.claim_id -StartedAt $startedAt -ReservedAt $reservationAt -ManifestSha256 $manifestSha256 -CollectorSha256 $collectorSha256 -ExpectedHost $ExpectedHost -AuthorizedSid $authorizedSid -Lock $claimLock
        $sshUsed = $false
        $failureReason = 'transport_failed'
        $transportStarted = $false
        try {
            $rawDocument = Invoke-Phase15OneSshTransport -ExpectedHost $ExpectedHost -CollectorBytes $collectorArtifact.Bytes -Claim $claim -ClaimId $claim.claim_id -ManifestSha256 $manifestSha256 -CollectorSha256 $collectorSha256 -ExpectedOutcomePath $OutcomePath -ReservedAt $reservationAt -Started ([ref]$transportStarted) -TransactionPath $transaction.JournalPath -Lock $claimLock
