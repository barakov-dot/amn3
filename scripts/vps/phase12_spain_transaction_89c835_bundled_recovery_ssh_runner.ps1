...
$expectedNonce = "89c8359e6142f03e8ab94baa9dcf3efbff34e5e5e4bb0bdd9a3692b3383ae9c8"
$expectedTransactionSha = "34454708115b48218bb8ec5ef6a1edce8ab6f401e22afa1bd41f38482cbf7b5f"
$expectedCapsuleSha = "dd6b6227349dc63aa04b1c06324a4cd51ff6a7e9cb3cc5c36fae3fb3e6396b0d"
$expectedDockerTreeSha = "b73ccebfde0aa4a21e47b3236e96f99b4c3801af2384cfd96d3b28fb161f1774"
...
if ($remoteHash.ExitCode -ne 0 -or $remoteText -notmatch "(?im)^$($expectedExecutorSha.ToLowerInvariant())  /root/amn2-spain-phase12-executor\.pyz$" -or $remoteText -notmatch "(?m)^$expectedExecutorBytes$") { throw "Remote 89C835 bundled recovery executor checksum mismatch." }
...
if ($remoteTransaction.ExitCode -ne 0 -or $remoteTransactionText -notmatch "(?im)^$($expectedTransactionSha.ToLowerInvariant())  $([regex]::Escape($transactionPath))$" -or $remoteTransactionText -notmatch "(?im)^$($expectedCapsuleSha.ToLowerInvariant())  $([regex]::Escape($capsulePath))$") { throw "89C835 bundled recovery transaction checksum mismatch." }
...
if ($cleanupResult.ExitCode -ne 0) {
    ...
    throw "89C835 manual cleanup failed; retained package tree was not accepted as safely removable."
}
$packageAbsent = Invoke-ExactSsh (@($sshBase + @($target, "test ! -e /opt/amn2-spain-package && test ! -L /opt/amn2-spain-package"))) ([byte[]]@())
if ($packageAbsent.ExitCode -ne 0) { throw "89C835 manual cleanup package absence verification failed." }
...
if ($terminalResult.ExitCode -ne 0) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($safeRemoteError)) { [Console]::Error.WriteLine($safeRemoteError) }
    throw "Transaction 89C835 terminal recovery failed; current transaction remains fail-closed."
}
