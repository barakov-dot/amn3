...
from scripts import phase16_bot_integration_validator_v3 as schema_validator
...
GATE_MANIFEST_RELATIVE = Path(
    "research/amn2/phase16-bot-integration-readback-gate-011-manifest-2026-09-23.json"
)
EVIDENCE_DIRECTORY = Path(
    "C:/Users/SooL/Documents/VPS-OPS-LAB/worktrees/"
    "phase16-bot-integration-readback-runner-20260922/execution-011"
)
LOCAL_REASONS = frozenset({
    'approval_binding', 'manifest_binding', 'evidence_directory', 'evidence_exists',
    'target_binding', 'transport_no_remote_receipt', 'receipt_binding',
    'receipt_reason', 'frame_size', 'payload_binding', 'output_cap',
})


def local_reason(error):
    if isinstance(error, (core.Stop, base.core.Stop, remote.RemoteStop)):
        code = error.args[0] if len(error.args) == 1 else None
        if type(code) is str and code in LOCAL_REASONS | remote.STOP_REASONS:
            return code
...
def build_payload(root=ROOT):
    root = Path(root)
    data = canonical(root / "scripts/vps/phase16_bot_integration_readback_v3.py", compile_python=True)
    manifest = canonical(root / schema_validator.SOURCE_MANIFEST_RELATIVE)
    payload = remote.MAGIC + f"{len(data):08d}{len(manifest):08d}".encode() + data + manifest
    remote.parse_payload(payload)
    return payload


def remote_script(root=ROOT):
    return canonical(Path(root) / "scripts/vps/phase16_bot_integration_readback_remote_v3.py",
                     compile_python=True)
...
def expected_gate_manifest(root=ROOT):
    root = Path(root)
    prior = base.validate_gate_manifest(base.gate_manifest(root), root)
    payload = build_payload(root)
    blobs = {
        "local_runner": canonical(root / "scripts/phase16_bot_integration_readback_gate_v3.py",
                                  compile_python=True),
        "schema_validator": canonical(root / "scripts/phase16_bot_integration_validator_v3.py", compile_python=True),
        "manifest_builder": canonical(root / "scripts/phase16_bot_integration_manifest_v3.py", compile_python=True),
        "base_runner": canonical(root / "scripts/phase16_bot_integration_readback_gate.py",
                                 compile_python=True),
        "remote_supervisor": remote_script(root),
        "portable_core": canonical(root / "scripts/vps/phase16_bot_integration_readback_v3.py",
                                   compile_python=True),
        "transport_helper": canonical(root / "scripts/phase16_bot_linux_gate.py",
                                      compile_python=True),
        "integration_manifest": canonical(root / schema_validator.SOURCE_MANIFEST_RELATIVE),
        "payload": payload,
    ...
    expected = {
        ...
        "amn3_base": "32983ef45a8ebe5a91c7d0046ba6a5705025c5f0",
        ...
        "limits": {**prior["limits"], "dependency_metadata_file_bytes": 262144,
                   "dependency_pth_file_bytes": 65536, "dependency_total_bytes": 8388608},
        "readback_scope": prior["readback_scope"],
        "command_contract": {
            "entrypoint": "scripts/phase16_bot_integration_readback_gate_v3.py",
...
def validate_gate_manifest(value, root=ROOT):
    core.require(value == expected_gate_manifest(root), "manifest_binding")
    return value
...
def frame_request(script, payload):
    ...
    bootstrap = (
        ...
        "exec(compile(b,'<bound-phase16-integration-readback-011>','exec'),{'__name__':'__main__'})"
    )
...
def validate_receipt_numeric(value, returncode):
    schema_validator.validate_receipt(normalize_numeric_signals(value), returncode)
    if value['status'] == 'STOP_NO_RETRY':
        core.require(value['reason'] in remote.STOP_REASONS, 'receipt_reason')
    return value
...
def diagnose_rejection(value, returncode):
    ...
    envelope["partial"] = {}
    try:
        schema_validator.validate_receipt(envelope, returncode)
    except Exception:
        return "envelope"
    partial = value.get("partial")
    if not isinstance(partial, dict) or not set(partial) <= PARTIAL_STAGES:
        return "envelope"
    manifest = schema_validator.source_manifest(ROOT)
    for name in sorted(partial):
        try:
            schema_validator.validate_partial({name: partial[name]}, manifest)
        except Exception:
            return name
    return "envelope"
...
def execute_once(evidence_dir, *, approval, approved_remote_sha,
    ...
    try:
        ...
        except (core.Stop, base.core.Stop):
