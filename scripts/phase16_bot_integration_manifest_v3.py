...
def schema_allowlist(texts):
    ...
    """
    tables, indexes, triggers = {}, set(), {}
    for text in texts:
        tree = ast.parse(text)
        dynamic = {id(part) for value in ast.walk(tree) if (isinstance(value, (ast.JoinedStr, ast.BinOp)) or
                   (isinstance(value, ast.Call) and not
                    (isinstance(value.func, ast.Attribute) and value.func.attr in ('execute','executescript')) and not
                    (isinstance(value.func, ast.Name) and value.func.id == '_ensure_column')))
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                ...
                for match in re.finditer(r'\bALTER\s+TABLE\s+([a-z_][a-z0-9_]*)\s+ADD\s+COLUMN\s+([a-z_][a-z0-9_]*)\s+(?:INTEGER|INT|TEXT|REAL|BLOB|NUMERIC)\b', sql, re.I):
                    tables.setdefault(match[1], set()).add(match[2])
                for match in re.finditer(r'\bCREATE\s+TRIGGER\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-z_][a-z0-9_]*)\s+(?:BEFORE|AFTER|INSTEAD\s+OF)\s+(?:INSERT|DELETE|UPDATE(?:\s+OF\s+[a-z_][a-z0-9_]*(?:\s*,\s*[a-z_][a-z0-9_]*)*)?)\s+ON\s+([a-z_][a-z0-9_]*)\b', sql, re.I):
                    core.require(match[1] not in triggers or triggers[match[1]] == match[2], 'schema_parse')
                    triggers[match[1]] = match[2]
    ...
    return {'tables': {k: sorted(v) for k,v in sorted(tables.items())}, 'indexes':sorted(indexes), 'triggers':dict(sorted(triggers.items()))}
...
def build_manifest(root):
    ...
    return {'schema':'phase16.integration-manifest.v2','source_commit':SOURCE_COMMIT,
