...
APPROVAL = "PHASE16_ACTUAL_INTEGRATION_READBACK_20260923_010"
GATE_MANIFEST_RELATIVE = Path(
    "research/amn2/phase16-bot-integration-readback-gate-010-manifest-2026-09-23.json"
)
EVIDENCE_DIRECTORY = Path(
    "C:/Users/SooL/Documents/VPS-OPS-LAB/worktrees/"
    "phase16-bot-integration-readback-runner-20260922/execution-010"
)
...
def build_payload(root=ROOT):
    root = Path(root)
    data = canonical(root / "scripts/vps/phase16_bot_integration_readback_v2.py", compile_python=True)
    manifest = canonical(root / base.SOURCE_MANIFEST_RELATIVE)
...
def remote_script(root=ROOT):
    return canonical(Path(root) / "scripts/vps/phase16_bot_integration_readback_remote_v2.py",
                     compile_python=True)
...
def expected_gate_manifest(root=ROOT):
    ...
    blobs = {
        "local_runner": canonical(root / "scripts/phase16_bot_integration_readback_gate_v2.py",
                                  compile_python=True),
        "base_runner": canonical(root / "scripts/phase16_bot_integration_readback_gate.py",
                                 compile_python=True),
        "remote_supervisor": remote_script(root),
        "portable_core": canonical(root / "scripts/vps/phase16_bot_integration_readback_v2.py",
                                   compile_python=True),
    ...
    expected = {
        ...
        "amn3_base": "077e16fc2fa5c0f87ede04dfe2a9e5f82a7e7542",
        ...
        "command_contract": {
            "entrypoint": "scripts/phase16_bot_integration_readback_gate_v2.py",
...
def frame_request(script, payload):
    ...
    bootstrap = (
        ...
        "exec(compile(b,'<bound-phase16-integration-readback-010>','exec'),{'__name__':'__main__'})"
    )
