...
- Текущий результат 2026-06-01: dry-run/audit metadata local slice перенесен на fresh VPS-gate candidate `codex/remote-operation-vps-gate-prep` и обновлен поверх текущего `codex-vps-test-prep`; focused/docs suite `107 passed`, full suite `572 passed`.
...
- Текущий результат 2026-06-01: local-only lifecycle gate выполнен в `amn2/codex/api-token-lifecycle-gate`, commit `c2ba646`; stacked branch поверх route/auth binding доступен как `amn2/codex/api-token-lifecycle-gate-stacked`, commit `256d0c0`; stacked verification: `56 passed`, full suite `555 passed`; `/api/*` routes, `config:read`, write/remote-exec scopes не добавлялись.
- Следующий шаг: не добавлять `/api/*`, `config:read` или write scopes до VPS evidence, route policy entry, privacy/secret-read classification и отдельного route-exposure решения.
