...
- Текущий результат 2026-06-01: local-only lifecycle gate выполнен в `amn2/codex/api-token-lifecycle-gate`, commit `c2ba646`; stacked branch поверх route/auth binding доступен как `amn2/codex/api-token-lifecycle-gate-stacked`, commit `256d0c0`; `/api/*` routes, `config:read`, write/remote-exec scopes не добавлялись.
