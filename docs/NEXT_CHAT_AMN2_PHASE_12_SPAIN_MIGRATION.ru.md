...
29 committed, 6 removed, 5 unrecorded, all recorded stages blueprint-equal.
The exact retained tree hashes are `/opt` `0F2F2ADE…D25119`, `/etc`
`CD947553…CAF5A7`, `/var` `8F95E1F1…27BC99`. Only the dedicated AMN2 Docker
unit is active/enabled. Foreign service and USA remain unchanged.

Next gate is exact audit-bound terminal resume: executor
...
It removes only the audited AMN2 contour and returns dynamic foreign-equality
receipt. Scoped verification: `254 passed, 4 skipped`; diff check passed.
...
install attempt.
# Current override 2026-07-25: v21 recovered; fresh v22 ready

Fresh v21 transaction
`08aaf19769b06723a826b63b39235bd848fde5db5e2c5cd659516186c9a8fab3`
failed closed after Docker auto-start exposed exact healthy AWG before the
interface intent was recorded. Checksum-bound manual cleanup and terminal
recovery passed: the exact AMN2 contour is absent, foreign persistent equality
is true, volatile counts are `0/0`, and USA is unchanged.

The fix adopts only the exact desired AWG active identity and issues no
start/restart during adoption. Fresh v22 is double-built byte-equal and
clean-room verified: package
...
Checksum-bound cleanup and terminal recovery passed: AMN2 contour absent,
foreign persistent equality true, volatile `0/0`, USA unchanged.

Only the AWG active action now defers initial/pending transient observation
errors into its existing bounded readiness; all other actions remain strict and
no start/restart is issued for an already-running container. Phase 12 scope:
`327 passed, 4 skipped`. Fresh v23 is double-built byte-equal and clean-room
verified: package
...
# Current override 2026-07-26: v24 recovered; fresh v25 ready
...
equality is true, volatile counts are `0/0`, and USA is unchanged.
...
was later exact-observed and safely removed by rollback. Fresh v25 performs a
final exact observation after every idempotent error, including the last one;
only exact desired identity is accepted, while strict categories still fail
immediately.

Fresh v25 is double-built byte-equal and clean-room verified: package
...
`8BC5375F244F7CDD77A12BD4173CA19BE7430C35E49756D7B846906719369F43`.
Source remains `55dc243b8e6c6bdb57f8301b56326e4cd4072d19`; Phase 12 scope is
`329 passed, 4 skipped`. Seven verified 20 MiB upload parts are ready. Install
runner SHA-256 is
...
# Current override 2026-07-26: v25 recovered; fresh v26 ready

Fresh v25 transaction `af340c8f4a24e40c00d1e659030c5ab1ec9fd0303a772cd8411a14965a462e49`
failed closed with `docker_retry_exhausted`. Approved checksum-bound manual
cleanup and terminal recovery passed: the AMN2 contour is absent, foreign
persistent equality is true, volatile counts are `0/0`, and USA is unchanged.

The retained ledger proved the exact AWG container became observable only
after the final immediate post-error observation; rollback reconciliation then
exact-adopted and safely removed it. Fresh v26 uses the existing bounded AWG
readiness policy (`60 x 0.25s`) as the post-failure exact-observation window
before Docker retry exhaustion. Only exact desired identity is accepted; a
running AWG container is never restarted or recreated, and failure retains
exact automatic rollback.

Fresh v26 is double-built byte-equal and clean-room verified: package
...
`8BC5375F244F7CDD77A12BD4173CA19BE7430C35E49756D7B846906719369F43`.
Source remains `55dc243b8e6c6bdb57f8301b56326e4cd4072d19`; Phase 12 scope is
`330 passed, 4 skipped`. Seven verified 20 MiB upload parts are ready; only
...
## Current override 2026-07-27: v30 fail-closed; AD0E8B recovery ready
