...
| Порядок stage и rollback | [coordinator](../scripts/vps/phase16_controlled_stage_coordinator.py): `validate_stage_request`, `execute_stage`, `classify_stage_failure` | Распаковка и перенос пакета на target, claims/milestones, запуск stage; при `recovery_required` cleanup пропускается. `rollback_failed` — ошибка cleanup, `rolled_back` без readback не доказывает восстановление; см. [recovery-контракт v1](superpowers/specs/2026-09-08-phase16-controlled-stage-recovery-contract.ru.md) |
| Recovery metadata compatibility | [parser](../scripts/vps/phase16_recovery_metadata.py): `parse_canonical`, `classify_metadata` | Только переданные данные и exact bindings; без IO/SSH/cleanup. valid означает форму, не ownership/quiescence/readback |
...
| Application staging ownership и cleanup | [ownership tests](../tests/test_phase16_application_staging_ownership.py) | Shell на временных fixtures; не live rollback и не проверка POSIX modes на Windows |
| Recovery metadata schemas и согласованность outcome/milestones | [parser tests](../tests/test_phase16_recovery_metadata.py) | Синтетические входы и offline coordinator harness; не live recovery |
