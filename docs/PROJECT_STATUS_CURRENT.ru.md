# Навигационное предупреждение — 2026-09-07

Актуальный вход: [START_HERE](START_HERE.ru.md). Подробный статус активной фазы
находится в указанном там execution plan, а не в этом накопительном журнале.
Весь текст ниже сохранён как история: прежние «текущие» приоритеты, GO и approvals
не являются разрешением новой задаче. Старый путь сохранён для совместимости ссылок.
...
на `E118EA7B299E96F891C92309AC420A44B6E8254FFA0E1A91A4BFCD614A3FFABD`;
уже отсутствующий primary group отмечен removed. Run009-to-current и current
before/after persistent foreign-service equality равны `true`, volatile counts
`0/0`. Старый AMN2 contour отсутствует; install после finalize ещё не запускался.

Fresh v21 package/executor с bounded AWG readiness (один start, без restart)
собраны дважды byte-equal и offline verified: package
...
(`152752` bytes). Transaction `958e91` rollback завершён: AMN2 contour absent,
foreign persistent equality passed. Full Phase 12 suite: `325 passed, 4 skipped`;
PowerShell parse passed; install runner
`C32CB9C9DAFC94B36196F37AC218A2BF7655ED0984452BE37CA5C5FDD1D75957`.
Следующий gate — commit/push bindings, затем exact checksum-bound Spain install
approval.
...
прошёл и удалил только retained `/opt/amn2-spain-package`. Последующий exact
terminal recovery также прошёл:
`recovery_action=removed_verified_owned_objects`,
`foreign_service_persistent_equal=true`, volatile=`0/0`. Он удалил только
recorded current AMN2 contour. AMN2 не запускался; foreign Spain service не
останавливался/не изменялся; USA остаётся rollback contour.

Read-only разбор journal уточнил, что прежний `unsupported` относится к
containerd/snapshotter строкам и сам по себе не является доказанной причиной
`docker image load`. Поэтому v15 не делает blind runtime change: он сохраняет
classic-vfs package и добавляет только fail-closed диагностические labels для
image load (`timeout`, `input_changed`, `output_exceeded`,
`command_failed`). Raw stderr и секреты по-прежнему не выводятся.

```text
v15_package_sha256=DAA40D48B88B2AFB0FC4A57A1E5313D8B2851BCED89AEC655B628CB859AEA585
v15_package_bytes=139970560
v15_manifest_sha256=F13A7C4A02F7B9233629AD06DF06265BB1FC84B69478B4BDB03F1484515C79F2
v15_executor_sha256=07E066F15FA671DBF9B9F74ECAD2373C00D4A7551972E316F51BCB8265B630CC
v15_executor_bytes=145791
v15_install_runner_sha256=762428179994934DE358F08CE55E0D6489E5095DD7C48742AA827B851C16AE9B
...
local_verification=double_build_byte_equal|offline_clean_room_69_artifacts|source_55dc243_verified|scoped_233_passed_4_skipped|powershell_parser_passed
next_gate=diff_review|commit_push_origin_readback|exact_v15_install_approval
```

# Предыдущий override 2026-07-24: v14 reached Docker, failed at `awg_image_loaded`; transaction `544db…` recovery gate
...
Separate nonpersistent diagnostic передаёт 16 MiB zero bytes только в remote
`/dev/null`, с `Compression=no` и 60-second bound. Он также timeout’нулся,
не вызывая AMN2, package/executor, install, rollback или foreign service.
Следовательно, 300-second package limit недостаточен: даже при верхней
границе observed throughput package `139970560` bytes требует более 500
seconds. Новый runner меняет только этот bound на 900 seconds; package,
executor, hashes, remote SHA gate и rollback остаются прежними.

```text
active_phase=AMN2 Phase 12 Spain Migration|capacity_bound_upload_local_gate_complete
...
fresh_install_executor_sha256=D792D9CABB6B7FE3FABD7BC4B07D833D27549FE1484900770B54214D38FEAC29
fresh_install_executor_size=145505
fresh_install_runner_sha256=172A0FBA9E9FB403D205EF40D9A3CB6A12A247A0AF29DB3FE9F4848E54133E6D
...
Следующий approved attempt также остановился до executor: SCP вернул success,
но remote SHA check fail-closed, потому что upload использовал generic local
basenames, а verifier ожидал AMN2-bound temporary `-a` paths. Исправление
...
Legacy-SCP upload прошёл и executor дошёл до install, но вернул
`production runtime rollback failed`. Повтор install запрещён. Pinned read-only
failure observation подтвердил inactive AMN2/Docker units и retained exact owned
contour; current transaction `1d7511…` имеет `manual_recovery_required`.
Её bound Docker tree: SHA `605192…`, 916 entries, 41,902,300 bytes, one
filesystem, recorded block rdev `64770`. Следующий обязательный action —
manual cleanup только `/opt/amn2-spain-package`, с remote executor SHA и
transaction SHA preconditions; terminal rollback/equality идёт лишь после этого.

Current manual cleanup passed: receipt `8b7bf914…`, current transaction SHA
`08f1…`, status остался `manual_recovery_required`; post-check подтвердил
отсутствие package tree и inactive AMN2/Docker units. Prepared next runner
требует remote executor, transaction/capsule SHA и package-tree absence, затем
выполняет только sealed terminal rollback текущего contour и foreign equality.
...
spain_unrelated_service=untouched
usa_rollback_contour=unchanged
next_gate=commit_push_origin_readback_then_exact_current_terminal_recovery_approval|no_install_retry_before_foreign_equality_receipt
```
...
# Текущий override 2026-07-26: v22 rollback закрыт; fresh v23 готов

Fresh v22 transaction
`49da84044f816b72f13cad6d6e65529748606af54050eabd6a591cea38d4b339`
fail-closed остановилась на initial AWG health probe до interface intent.
Checksum-bound cleanup и terminal recovery прошли: AMN2 contour absent,
foreign persistent equality `true`, volatile `0/0`, USA не изменялась.

Root cause: auto-started container был running, но transient health error
возникал на initial и pending observation раньше существующего bounded readiness.
Только AWG active action теперь defer-ит эти две ошибки в bounded readiness;
остальные actions остаются strict, start/restart при уже running container не
выполняется. Full Phase 12 scope: `327 passed, 4 skipped`. Fresh v23 double-build
byte-equal и clean-room verified: package
...
# Текущий override 2026-07-26: v23 rollback закрыт; unified v24 готов

Fresh v23 transaction
`a8a30f5a0759562706213325e02823e10abd4f4994dfca90753d9e7ee01a8669`
fail-closed остановилась после committed Docker container/network и до записи
`interface:awgsp0` intent. Checksum-bound cleanup и terminal recovery прошли:
AMN2-контур отсутствует, foreign persistent equality `true`, volatile `0/0`,
USA не изменялась.

Unified v24 записывает durable interface intent до fallible AWG probe, принимает
только exact healthy AWG identity, использует bounded readiness `60 x 0.25s`,
не перезапускает и не пересоздаёт running container. Для idempotent Docker,
systemd, network и web stages допускается максимум одна повторная попытка с
allowlisted cause/attempt receipt; любая неуспешная стратегия завершает exact
automatic rollback. Transport выбирает exact artifact cache, затем verified
seven-part 20 MiB cache, затем bounded whole-file upload.

Fresh v24 double-build byte-equal и clean-room verified: package
...
# Текущий override 2026-07-27: v28 rollback закрыт; fresh v29 готов

Fresh v28 дошла до `host_network_applied`, после чего composite start
`amn2-spain-network.service` fail-closed перешёл в rollback. Transaction
`34454708115b48218bb8ec5ef6a1edce8ab6f401e22afa1bd41f38482cbf7b5f`
закрыта checksum-bound bundled recovery: AMN2-owned contour отсутствует,
все AMN2 units inactive, foreign persistent equality `true`; посторонний
Spain-сервис и USA rollback contour не изменялись.

Fresh v29 сохраняет systemd-first путь, а при первом отказе применяет exact
network contour через durable controller ledger и делает ровно один повторный
start. Running/exact service принимается без restart; любой второй отказ
завершается `network_retry_exhausted` с automatic exact rollback.

Fresh v29 double-build byte-equal и clean-room verified: package SHA-256
...
(`156056` bytes). No-follow clean-room extract: 69 regular entries, 141 source
files, exact source `55dc243b8e6c6bdb57f8301b56326e4cd4072d19`. Full Phase 12 scope:
`387 passed, 4 skipped`; PowerShell AST и diff check passed. Семь verified
20 MiB parts готовы. Install runner SHA-256
`F37013B32E46EA6A3B7988604C00519477260C4CB9DA5F64B5CBE21F3C45304D`.
Следующая live-операция — fresh checksum-bound v29 install.

# Текущий override 2026-07-27: v29 fail-closed; recovery готов

Fresh v29 transaction
`75f0f66c438d4115c0bf7640bdfbfec1c320ac8f06aac36229ad7681de0f1df2`
завершилась `manual_recovery_required`. Runtime/network/Docker objects удалены,
четыре AMN2 units inactive, AMN2 nft table и route отсутствуют. Retained ledger
SHA-256 `5C7FEFC65270C670B9E581F740D474BE820F3B1AEA3A145F1C40E11B93853841`
содержит девять committed identity/filesystem objects.

Checksum-bound bundled recovery runner SHA-256
`E7DA04FC21841152C90A57B3CAB706E4207D0B4DA0530122D815BED7F4DE18AA`
привязан к nonce `9d2d0d0b3bae9df38a717efd8995e4715893ec6ff861eb4824510ed9aa201889`,
capsule SHA-256
`9D912BD1D9BED6DD1C2678E6B4A8EE7B26D74ED9592B4B405DED82184427FD7C`
и Docker tree SHA-256
...
# Текущий override 2026-07-27: v30 fail-closed; AD0E8B recovery готов

Fresh v30 дошла до production runtime и завершилась
`production runtime rollback failed`. Transaction SHA-256
`6DAE7CD3B2D6AF6534FB6B0D66269E8E961B7ACF8DF915C0398935098DE7C0B7`
имеет status `manual_recovery_required`, nonce
`ad0e8bc5f43f2cb4de3958d7ac151f5d4242ea18d0707d6e6696e0f6909b6375`.
Все четыре AMN2 units inactive; retained package/identity/filesystem contour
остаётся fail-closed. Docker tree read-only bound: SHA-256
...
привязан к capsule SHA-256
`94E41971ABB6BCC01F3105D1CE6DDF8106CD5B5E7DD6AD44770E32C274727561`
и current v30 executor. Он выполняет только verified package cleanup, exact
