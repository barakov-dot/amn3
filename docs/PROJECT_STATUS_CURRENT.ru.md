...
Phase 4 `P4-NG` Named Gate / Write API Readiness was started as AMN3 docs-only planning on 2026-06-10. Plan: `docs/superpowers/plans/2026-06-10-p4-ng-named-gate-write-api-readiness.md`. Charter/evidence: `research/amn2/phase-4-ng-gate-charter-and-plan-2026-06-10.md`. `NG-C001` named gate charter and `NG-C002` safety boundary restatement are closed and removed from the active plan. Active next recommendation is `NG-C003` secrets policy for gate outputs and `NG-C004` go/no-go format for all gates. No AMN2 code, live VPS command, SSH command, route expansion, public exposure, config delivery, `/api/clients` CRUD, Local Agent mutation, backup/import/reboot, token issue/revoke/rotate API route or production peer/user mutation was performed. `NG-V001` read-only VPS baseline gate is not authorized until the operator explicitly approves that gate and provides the SSH target outside repo secrets; write API live work remains blocked until a separate `P4-WRITE-API-LIVE-GATE`.
