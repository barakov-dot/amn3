...
- Config templates показывает шаблоны клиентских конфигов и `vpn://` preview.
...
Для peer из Amnezia доступно действие `Пометить как созданный в Amnezia`. Оно только сохраняет пометку в базе, не удаляет peer и не меняет конфиг AmneziaWG. Для локального устройства без peer доступно действие `Добавить в Amnezia`: оно расшифровывает сохраненный preshared key, добавляет peer с прежним IP/public key в AmneziaWG и требует `VPS_APPLY_ENABLED=true`.
