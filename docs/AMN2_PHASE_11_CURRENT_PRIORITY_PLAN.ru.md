...
Актуально: 2026-07-14 после `PHASE11-TELEGRAM-002`, `PHASE11-OPS-001`,
`PHASE11-RECOVERY-001` и аудита второго VPS.
...
Текущий шаг: только review exact gate, scripts, rollback/cleanup contract и
secret-handling path. Сам live rehearsal требует нового точного approval.
...
```text
GPT-5.6 SOL -> REVIEW_PHASE11_RESTORE_001A_CANONICAL_FULL_SECRET_DISPOSABLE_REHEARSAL_GATE -> PREPARE_EXACT_RESTORE_001A_APPROVAL_AND_ROLLBACK_CLEANUP_CONTRACT
```

Тройная:

```text
GPT-5.6 SOL -> REVIEW_PHASE11_RESTORE_001A_CANONICAL_FULL_SECRET_DISPOSABLE_REHEARSAL_GATE -> PREPARE_EXACT_RESTORE_001A_APPROVAL_AND_ROLLBACK_CLEANUP_CONTRACT -> AFTER_APPROVAL_RUN_VERIFY_CLEANUP_AND_REAUDIT_WITH_PRODUCTION_AWG_UNTOUCHED
```

Более — рекомендовано:

```text
GPT-5.6 SOL -> REVIEW_PHASE11_RESTORE_001A_CANONICAL_FULL_SECRET_DISPOSABLE_REHEARSAL_GATE -> PREPARE_EXACT_RESTORE_001A_APPROVAL_AND_ROLLBACK_CLEANUP_CONTRACT -> AFTER_APPROVAL_RUN_VERIFY_CLEANUP_AND_REAUDIT_WITH_PRODUCTION_AWG_UNTOUCHED -> DECIDE_OLD_FALLBACK_AND_SECOND_VPS_SAFE_RETIREMENT_GATES -> IMPLEMENT_TELEGRAM_002A_LOCAL_PERSISTENT_ADMISSION_AND_UNIT_HARDENING
```
