...
API/web-panel slice от 2026-06-04 реализован, запушен и fast-forward merged в stable `codex-vps-test-prep`, commit `294803e Add API readiness and token web pages`: web-admin-only страницы `API readiness` и `API token lifecycle` поверх уже verified baseline. Implementation evidence: `research/amn2/api-web-panel-finish-implementation.md`; AMN3 VPS install/update package пересобран от `294803e`; real VPS API/web-panel gate passed 2026-06-04, `run_id=20260604T102355Z`, evidence `research/amn2/api-web-panel-vps-evidence-2026-06-04.md`. Границы остаются прежними: без `/api/clients` write CRUD, без API `config:read`, без public/self-service config delivery, без live peer apply/revoke и без публикации web/API наружу.
...
- fresh candidate поверх текущего `codex-vps-test-prep`: `codex/remote-operation-vps-gate-prep`, head `7281254`;
- VPS gate runbook: `research/amn2/vps-gate-remote-operation-dry-run-audit.md`;
- full local suite для fresh candidate: `603 passed, 1 warning`.
...
13. Read-only metrics privacy classification: prepared in `research/amn2/read-only-metrics-privacy-classification.md`; first aggregate-only route shell implemented in `amn2/codex/read-only-api-route-shell`, head `2010d60`, and real VPS loopback API smoke passed.
...
3. Real VPS loopback API smoke passed 2026-06-02 with scoped token issue/use/revoke and no raw token/header/hash/config/keys/PSK in AMN3 evidence.
...
4. Read-only route shell is implemented in `amn2/codex/read-only-api-route-shell` and merged into stable `codex-vps-test-prep` at `5f12736`; next lifecycle work must start from the stable baseline and stay behind separate route/secret gates.
5. `config:read`, write, remote-exec, destructive, backup/import and broad admin-equivalent bearer tokens remain blocked.

## Recommendation

API integration is now the priority product lane from `VPN Ops Lab — KYORESUAS-API`, and it entered `amn2` as a native, scoped, read-only route shell in `codex/read-only-api-route-shell`, not as copied KYORESUAS code. The VPS loopback API smoke gate is passed and the branch is merged into stable `codex-vps-test-prep` at `5f12736`. Controlled real VPS verification remains mandatory before any API route that calls SSH, syncs peers, emits config or changes runtime state.
