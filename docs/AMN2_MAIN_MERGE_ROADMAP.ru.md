...
Следующий API/web-panel slice от 2026-06-04: не пересобирать read-only API shell, а добавить web-admin-only страницы `API readiness` и `API token lifecycle` поверх уже verified baseline. Implementation plan: `docs/superpowers/plans/2026-06-04-amn2-api-web-panel-finish.md`. VPS инструкция для будущего теста: `docs/AMN2_API_WEB_PANEL_VPS_TEST_RUNBOOK.ru.md`. Границы остаются прежними: без `/api/clients` write CRUD, без API `config:read`, без public/self-service config delivery, без live peer apply/revoke и без публикации web/API наружу.
