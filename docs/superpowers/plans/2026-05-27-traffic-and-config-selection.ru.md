...
**Цель:** добавить foundation для статистики трафика и выбора версии AmneziaWG-конфига (`amneziawg_v1_5` или `amneziawg_v2`) для пользовательских и административных сценариев Telegram-бота.

**Архитектура:** сначала вводится единый слой версий конфигов и renderer dispatch. Затем расширяется SQLite schema и repository traffic snapshots. После этого `AccessService` принимает выбранную версию, а отдельный traffic service записывает статистику по peer public key через fake-friendly collector interface.

**Tech Stack:** Python 3.12+, pytest, SQLite, dataclasses, текущие repository/service patterns.
