...
Approval использована ровно один раз. Outcome `spain-fresh-20260721-008`
завершился fail-closed после создания claim из-за доказанного локального CRLF
stdin defect. Literal consumed; повторять его нельзя.
...
Повторять literal нельзя. Failure evidence может содержать
...
Run 008 доказал локально исправимую причину. Допускается ровно один новый
checksum-bound run 009 после отдельного review и approval. После него
используется provider console/иной подход; run 010 не допускается.
