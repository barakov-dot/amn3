...
Перед выбором следующей локальной задачи сверяемся с `docs/AMN3_PRE_VPS_LOCAL_STATUS_MATRIX.ru.md`: она отделяет уже
реализованный read-only слой от post-VPS планов и помогает не дублировать соседний VPS smoke.
