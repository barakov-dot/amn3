...
- `P6-C009` Named live apply/smoke gate for AMN2 `c46f664` on current disposable VPS `89.185.80.166`: optional, not opened. Complexity: high/live. Depends on exact operator phrase, checksum verification, source overlay apply, read-only loopback API smoke, web/bot runtime verification and safe evidence. Must not open public exposure, config delivery, write API, Local Agent mutation, backup/import/reboot, destructive cleanup or Telegram identity mutation.

Active default critical tasks: none after `P6-C005`. Critical gated/deferred work remains `P6-C001`, `P6-C002`, `P6-C003`, `P6-C004`, `P6-C007`, `P6-C009`, `VPS-REBUILD-001`, Local Agent write/config routes and production peer/user mutation.
