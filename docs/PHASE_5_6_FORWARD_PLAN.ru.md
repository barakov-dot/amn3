...
- `P6-C007` Destructive cleanup/reinstall gate for current working VPS `89.185.80.166`: deferred until the operator explicitly decides to assemble/test the clean installer. Complexity: high/destructive. Depends on separate named destructive gate, explicit retention/data-loss decision, stop criteria and operator acceptance.
...
- `P6-I007` Interactive fresh-install wizard/bootstrap automation: local-only code/docs/tests for future question-and-answer clean installer with safe defaults, preflight validation, dry-run output, operator-provided secrets and no live/destructive execution by default.
