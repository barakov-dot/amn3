...
Готово: scoped token model, `/api/servers`, `/api/servers/{server_name}/summary`, `/api/metrics/summary`, `/api/users/summary`, `/api/integration/status`, `/api/local-agent/runtime/summary`, API smoke CLI, safe `api smoke-cycle`, audit and forbidden marker checks.
