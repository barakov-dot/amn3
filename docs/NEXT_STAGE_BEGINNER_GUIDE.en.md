...
Bot button labels and core bot messages are stored in a text catalog. Russian is
the default language, and English remains available as a fallback for future
...
After that, the approve flow is guarded: if adding the peer on the VPS fails,
the bot does not send the config to the user and shows the admin an error
without secrets.
