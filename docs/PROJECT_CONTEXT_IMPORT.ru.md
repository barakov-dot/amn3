...
Для новой задачи использовать [START_HERE](START_HERE.ru.md) и активный план,
указанный в нём. Ниже сохранён исторический context packet; не импортировать его
как текущий статус и не выполнять прежние next steps/approvals автоматически.
Исторический текст и прежний путь сохранены, новые результаты добавляются в свои источники.
...
`research/amn2/phase-8-helper-style-hardening-2026-06-22.md`. Будущие
PowerShell helpers должны использовать ASCII prompts или UTF-8 with BOM,
`${TargetIp}:PORT` или `$($TargetIp):PORT` для URL interpolation, проходить
parse check и probe URL dry inspection перед operator handoff. Live action не
выполнялся.

Phase 8 private RC next-chat sync is complete docs-only. Handoff:
`docs/NEXT_CHAT_AMN2_PRIVATE_RC_SESSION_0.ru.md`. Evidence:
`research/amn2/phase-8-private-rc-next-chat-sync-2026-06-22.md`. Use it as the
first read in a new chat after `PRIVATE_RC_OPERATOR_RUN_GATE`. It records final
`passed-read-only` status, latest heads, proven/unproven scope, stop-lines and
next exact gates menu. No live action was performed. Default next step:
`ЖДАТЬ_ЗАПРОСА_ОПЕРАТОРА`; practical next step if Android phone is available:
