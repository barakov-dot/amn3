...
Phase 6 `P6-C006` local package preflight for AMN2 `b3102db` is complete as `package-ready-not-vps-smoked`. Evidence: `research/amn2/phase-6-final-vps-refresh-package-b3102db-2026-06-13.md`. Built `dist/amn2-vps-update-and-smoke-kit-b3102db.zip`, sha256 `B4C3FF33FD0A721C97A83EA8AF08D5E5B6EA5E8D1862EEB63494E8842D56A21B`, from source zip `dist/amn2-codex-vps-test-prep-b3102db-source.zip`, sha256 `72342DB625D53AE2F6B68835A1FC4E080684A4A1E9018E791820899BB9A09778`. Package hygiene/test-extract passed with 5 kit entries, 329 source entries, required entries present, forbidden source entries absent, shell scripts LF/no-BOM and commit bindings present. CPython 3.12.13 toolchain check passed; fresh pytest was not run in this package step because the available CPython 3.12 bundled runtime lacks pytest. Latest VPS-smoked/package head remains `2215761 Polish operator web admin UX`; the remaining `P6-C006` live apply/smoke path still requires a separate named live gate. No live VPS command, SSH command, package upload/apply on VPS, source overlay, service restart/deploy, live bot verification/send, public exposure, config delivery, write API, Local Agent mutation, backup/import/reboot, production peer/user mutation, destructive provider action, Telegram token use, Telegram profile mutation, secret-bearing evidence publication or upstream/GPL code copy was performed.

Phase 6 `P6-M004` iOS AmneziaWG import/connectivity diagnostic boundary, `P6-X001` Public product copy polish and `P6-X002` Brand/media consistency are now closed as AMN2 local-only code/tests/docs. Evidence: `research/amn2/phase-6-client-compatibility-copy-boundary-2026-06-13.md`. AMN2 current branch head is `b3102db Add client compatibility delivery boundary`, pushed to `amn2/codex-vps-test-prep`; latest VPS-smoked/package head remains `2215761 Polish operator web admin UX`, and `b3102db` is package-ready locally but not VPS-smoked. The slice adds explicit client roles for iOS DefaultVPN as the primary RF-available iOS path, iOS AmneziaWG/Apple as an installed/legacy path, and Android AmneziaWG as a separate supported path; aligns Telegram delivery copy, web Config templates copy, API/web `/integration-status`, README and setup docs; keeps `.conf` as the first fallback; and records short one-tap tokenized config delivery links as part of `P6-C002`. Verification: RED client/status tests, focused `26 passed, 1 warning`, expanded `290 passed, 1 warning`, AMN2 `git diff --check` and staged check passed. No live VPS command, SSH command, package apply/rebuild, service restart/deploy, public exposure, config delivery, write API, Local Agent mutation, backup/import/reboot, production peer/user mutation, destructive provider action, Telegram token use, live bot send, Telegram profile mutation, secret-bearing evidence publication or upstream/GPL code copy was performed. `P6-M004`, `P6-X001` and `P6-X002` are removed from active Phase 6 plan. Next recommendation: `P6-C006` live apply/smoke gate for `b3102db` only if the operator explicitly opens the named live gate.
...
AMN2 source-overlay/package head: 2215761 Polish operator web admin UX (VPS-smoked in P5-C010)
AMN2 current branch head: b3102db Add client compatibility delivery boundary
AMN2 latest VPS-smoked package: dist/amn2-vps-update-and-smoke-kit-2215761.zip, sha256 6C360E8005E117EC59DD2829E9C4E9D2F36B5070275CD989D9D51A0675CF8B44, live-update-smoke-pass
AMN2 package/smoke status for b3102db: package-ready-not-vps-smoked; package dist/amn2-vps-update-and-smoke-kit-b3102db.zip sha256 B4C3FF33FD0A721C97A83EA8AF08D5E5B6EA5E8D1862EEB63494E8842D56A21B; source zip dist/amn2-codex-vps-test-prep-b3102db-source.zip sha256 72342DB625D53AE2F6B68835A1FC4E080684A4A1E9018E791820899BB9A09778; latest VPS-smoked package/source remains 2215761
