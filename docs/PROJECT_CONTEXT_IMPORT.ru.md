...
P4-NG default docs-only cosmetic queue: closed after NG-X002; next explicit decision is NG-V001 read-only VPS baseline gate only after named approval
P4-NG live/write status: NG-V001 read-only VPS baseline gate requires explicit operator approval and target SSH alias/host outside repo secrets; write API live work remains blocked until separate P4-NG-WRITE-API-LIVE-GATE; selected WAPI work remains docs-only/local-only with live_write_authorized: no
