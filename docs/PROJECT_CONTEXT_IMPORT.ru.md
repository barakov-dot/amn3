...
P4-NG default docs-only cosmetic queue: closed after NG-X002; NG-SC001 security checkpoint is closed; NG-V001 is opened as defer-awaiting-target
P4-NG live/write/destructive status: NG-V001 read-only VPS baseline gate has operator start approval recorded, but no SSH/VPS command has run; target SSH alias/host outside repo secrets and security_risk_decision: go are still required; write API live work remains blocked until separate P4-NG-WRITE-API-LIVE-GATE; destructive VPS rebuild remains blocked until separate VPS-REBUILD-001; selected WAPI work remains docs-only/local-only with live_write_authorized: no
