...
P4-NG default docs-only cosmetic queue: closed after NG-X002; NG-SC001 security checkpoint is closed; next explicit decision is NG-V001 read-only VPS baseline gate only after named approval and security_risk_decision: go
P4-NG live/write/destructive status: NG-V001 read-only VPS baseline gate requires explicit operator approval, target SSH alias/host outside repo secrets and security_risk_decision: go; write API live work remains blocked until separate P4-NG-WRITE-API-LIVE-GATE; destructive VPS rebuild remains blocked until separate VPS-REBUILD-001; selected WAPI work remains docs-only/local-only with live_write_authorized: no
