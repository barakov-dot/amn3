...
43. `NG-V001` read-only VPS baseline gate opening; evidence: `research/amn2/phase-4-ng-v001-read-only-vps-baseline-gate-2026-06-10.md`; status: `opened-defer-awaiting-target`.
...
4. `NG-V001` read-only VPS baseline gate is opened as `defer-awaiting-target`; it requires target SSH alias/host outside repo secrets and `security_risk_decision: go` before any SSH/VPS action.
5. Write API live work remains blocked until a separate `P4-NG-WRITE-API-LIVE-GATE`.
6. Destructive fresh VPS rebuild remains blocked until a separate `VPS-REBUILD-001` gate.
