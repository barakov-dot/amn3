...
## Ручная проверка Windows Sandbox — 2026-09-11

Текущий статус: BUILD_PASS_PACKAGE_PASS_SANDBOX_IMPORT_PARTIAL_PASS.
Разделы выше сохраняют историческое состояние на 2026-09-09.

После повторявшегося разрыва сеанса пустая offline-песочница запустилась;
затем оператор успешно открыл исходный тестовый .wsb и AmneziaVPN.
Причина первоначального разрыва не установлена; изменение конфигурации
исходного launcher для исправления не потребовалось. Версия Sandbox по
read-only host query: MicrosoftWindows.WindowsSandbox 0.8.107.0.

| Проверка | Результат и источник |
| --- | --- |
| Запуск candidate в Sandbox | PASS, скриншот оператора с экраном добавления соединения |
| Native import preview | PASS, отдельное last_config.mtu = 1280 и raw MTU = 1280 видны на скриншоте 621db13b-6d52-4e4a-bbf8-bd83d90c386e |
| Имя после импорта | PASS, Neobyatnaya.NET без расширения/ручного переименования, скриншот 19311948-774e-437d-9d16-39908842520b |
| Имя после повторного открытия | PASS, скриншот b7ce0d36-e134-4035-9e2e-7e5a936908d6; затем оператор отдельно подтвердил закрытие через меню и повторное открытие |
| Сохранённое last_config.mtu после перезапуска | UNKNOWN, readback хранилища/экспорт не выполнялся |
| Все параметры / duplicate import | NOT_RUN, полная сохранность и повторный импорт не проверены |
| Tunnel / adapter MTU / connectivity | NOT_RUN, offline-тест; главный экран показывал отключённое состояние |
| AmneziaWG / DefaultVPN | NOT_RUN, отдельные приложения этим прогоном не проверены |

Скриншоты получены в текущей задаче; raw JSON и ключи в Git не переносились.
После проверки оператор закрыл Sandbox. Не считать этот результат исправлением
исторического Windows traffic FAIL или Task 4.5 quality FAIL. Candidate остаётся
локальной сборкой с PR #3113, не официальным релизом. Рабочая AmneziaVPN не заменена,
service installer не запускался. AWG2_UNTOUCHED; package016 immutable;
general issuance disabled; Spain stage/install не выполнялись.

Следующий локальный gate: полный parameter readback и duplicate behavior в
одноразовой среде, затем отдельные import checks AmneziaWG и DefaultVPN с
зафиксированными официальными сборками. Не повторять NAME_PASS без новой причины.
