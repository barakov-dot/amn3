...
- `VPS-REBUILD-001` fresh VPS rebuild gate is opened as docs-only preflight; see `research/amn2/vps-rebuild-001-fresh-vps-rebuild-gate-2026-06-10.md`. It remains `defer` and does not authorize wipe, reinstall, package apply, live commands, public exposure, config delivery, write API, backup/import/reboot or production mutation. Novice-safe preflight preserves provider snapshot by default unless the operator records another retention path; local AMN2 source/package precheck comes first, target secrets regenerate where possible, and external secrets pass only through operator local channel. Source/package precheck evidence `research/amn2/vps-rebuild-001-source-package-precheck-2026-06-10.md` selects older AMN2 `1508e3c4a100b76815b29f91757290f1266f813d`; package build/hygiene evidence `research/amn2/vps-rebuild-001-package-build-hygiene-2026-06-10.md` marks `dist/amn2-vps-update-and-smoke-kit-1508e3c.zip` as `package-ready-not-vps-smoked`; current AMN2 head is now `23f18ef`, so future package apply/rebuild must rebuild from the selected current head and rerun source/package precheck; provider snapshot/backup confirmation evidence `research/amn2/vps-rebuild-001-provider-snapshot-confirmation-2026-06-10.md` is `defer`: monthly backup plan enabled, created/restorable backup not confirmed, delete actions not planned.
...
required_gate: requires VPS gate
implementation_status: docs-only design boundary closed 2026-06-10 in NG-N001; no reconciliation implementation, attach/import/backfill, real target detection or route change
recommendation: use NG-N001 as report-only reconciliation boundary; keep real target detection behind read-only VPS named gate and attach/backfill behind separate write/backfill gate
```
...
required_gate: requires VPS gate
implementation_status: docs-only design boundary closed 2026-06-10 in NG-N003; no operation queue implementation, worker, runner, runtime route or live execution
recommendation: use NG-N003 as queue/cancel/retry/status boundary; keep real operations behind implementation, write and VPS gates
```
...
- `NG-N001` attach-existing-server read-only reconciliation gate design was closed in `research/amn2/phase-4-ng-n001-attach-existing-server-read-only-reconciliation-gate-design-2026-06-10.md`; it defines future report-only reconciliation phases, safe fields, forbidden leakage fields, attach/backfill boundaries, conflict handling and RED test requirements without implementing reconciliation, attach/import/backfill, real target checks or route changes.
...
- `P4-BOT-ONBOARDING-001` bot onboarding language/header was completed as AMN2 local-only in `research/amn2/phase-4-bot-onboarding-language-header-2026-06-11.md`; AMN2 commit `137d471` adds the supplied `NEOBYATNAYA-AMNZ-BOT.png`, `/start` language prompt, Russian default `users.locale` and selected-locale main menu. Support/news assets were later inventoried by `P5-M001` as future separate bot planning inputs only; admin panel asset boundary remains separate under `P5-M004`.
- `P5-I003` runtime/toolchain standardization was completed as AMN2 local-only in `research/amn2/phase-5-runtime-toolchain-standardization-2026-06-11.md`; AMN2 commit `578d91e` pins supported local runtime to CPython 3.12.x and adds `python -m app.toolchain check`.
- `P5-I002` external-only backfill rehearsal was completed as AMN2 local-only in `research/amn2/phase-5-external-only-backfill-rehearsal-2026-06-11.md`; AMN2 commit `23f18ef` adds JSON-based `device backfill-external` dry-run/apply-to-copy flow and keeps all imported records `external_only`.

Следующее решение: очередь default docs-only cosmetic закрыта, `NG-V001` read-only VPS baseline gate закрыт как `go`, `P4-BOT-ONBOARDING-001`, `P5-I003`, `P5-I002`, `P5-I004`, `P5-M003` и `P5-M001` закрыты. Активных P4-NG задач больше нет. Отдельный destructive gate `VPS-REBUILD-001` открыт как `opened-defer-awaiting-final-destructive-approval`; выбран novice-safe режим `preserve_snapshot_required` + `provider_snapshot_required` + regenerate/operator-local secrets, но `VPS-FRESH-DEPLOY-001` зафиксировал, что clean deploy возможен из repo/package plus operator-provided secrets, а `VPS-FRESH-DEPLOY-002` подготовил clean Ubuntu/no-domain service-mode runbook. Source precheck выбрал старый AMN2 `1508e3c4a100b76815b29f91757290f1266f813d`; package build/hygiene прошел и package status теперь `package-ready-not-vps-smoked`; current AMN2 head is `23f18ef`, so any future VPS package apply/rebuild must rebuild from the selected current head and rerun source/package precheck. Backup plan enabled, но created/restorable backup не подтвержден, delete actions не планируются. До retention-path decision, stop-criteria review и точной финальной фразы `GO VPS-REBUILD-001 WIPE TARGET` не предлагать VPS command. Если rebuild не выбирается, next safe local-only recommendation is `P5-M005` Bot media asset upload/apply boundary, then optional `P5-M004` admin panel header asset boundary, `P5-M002` client guidance QA or design boundary for `P4-PRVTPRO-REFRESH-003`. Не копировать GPL code.
