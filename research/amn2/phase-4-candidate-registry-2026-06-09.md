...
Следующее решение: очередь default docs-only cosmetic закрыта, `NG-V001` read-only VPS baseline gate закрыт как `go`. Активных P4-NG задач больше нет. Отдельный destructive gate `VPS-REBUILD-001` открыт как `opened-defer-awaiting-final-destructive-approval`; выбран novice-safe режим `preserve_snapshot_required` + `provider_snapshot_required` + regenerate/operator-local secrets, но `VPS-FRESH-DEPLOY-001` зафиксировал, что clean deploy возможен из repo/package plus operator-provided secrets, а `VPS-FRESH-DEPLOY-002` подготовил clean Ubuntu/no-domain service-mode runbook. Source precheck выбрал AMN2 `1508e3c4a100b76815b29f91757290f1266f813d`; package build/hygiene прошел и package status теперь `package-ready-not-vps-smoked`; backup plan enabled, но created/restorable backup не подтвержден, delete actions не планируются. До retention-path decision, stop-criteria review и точной финальной фразы `GO VPS-REBUILD-001 WIPE TARGET` не предлагать VPS command. Если rebuild не выбирается, держать P4-NG в paused/docs-maintenance state или открыть отдельный local-only design boundary для `P4-PRVTPRO-REFRESH-003`. Не копировать GPL code.
