...
Следующее решение: очередь default docs-only cosmetic закрыта, `NG-V001` read-only VPS baseline gate закрыт как `go`, `P4-BOT-ONBOARDING-001` закрыт. Активных P4-NG задач больше нет. Отдельный destructive gate `VPS-REBUILD-001` открыт как `opened-defer-awaiting-final-destructive-approval`; выбран novice-safe режим `preserve_snapshot_required` + `provider_snapshot_required` + regenerate/operator-local secrets, но `VPS-FRESH-DEPLOY-001` зафиксировал, что clean deploy возможен из repo/package plus operator-provided secrets, а `VPS-FRESH-DEPLOY-002` подготовил clean Ubuntu/no-domain service-mode runbook. Source precheck выбрал AMN2 `1508e3c4a100b76815b29f91757290f1266f813d`; package build/hygiene прошел и package status теперь `package-ready-not-vps-smoked`; current AMN2 head is `137d471`, so any future VPS package apply/rebuild must rebuild from the selected current head and rerun source/package precheck. Backup plan enabled, но created/restorable backup не подтвержден, delete actions не планируются. До retention-path decision, stop-criteria review и точной финальной фразы `GO VPS-REBUILD-001 WIPE TARGET` не предлагать VPS command. Если rebuild не выбирается, next safe local-only recommendation is runtime/toolchain standardization (`P5-I003`), then optional design boundary for `P4-PRVTPRO-REFRESH-003`. Не копировать GPL code.
