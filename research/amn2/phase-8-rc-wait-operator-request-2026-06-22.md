# Phase 8: ждать запроса оператора

Дата: 2026-06-22.

Статус: `active-wait-operator-request-docs-only`.

Scope: AMN2 удерживается в private/operator RC
`launch-ready-with-explicit-limitations` на основе существующих Phase 8
evidence. Live VPS/SSH command, destructive action, package upload/apply,
...
provider mutation, production peer/user mutation и secret-bearing output не
выполнялись.

## Созданный документ

```text
docs/AMN2_PRIVATE_OPERATOR_RC_WAIT_OPERATOR_REQUEST.ru.md
```
...
## Условие следующего действия

Следующее действие разрешено только после явного именованного gate от оператора.
