...
confirmation_scope: provider snapshot existence and recoverability, safe summary only
result: pending-operator-confirmation
snapshot_required: yes
snapshot_confirmation: pending
...
Return only this safe summary after checking or creating the snapshot in the provider panel:
...
If `provider_snapshot_exists=no` or the operator is unsure whether a snapshot can be restored, keep this step `defer`.

## Current Result

```text
provider_snapshot_confirmation: pending-operator-confirmation
next_required_operator_action: create or verify provider snapshot, then return safe summary fields only
