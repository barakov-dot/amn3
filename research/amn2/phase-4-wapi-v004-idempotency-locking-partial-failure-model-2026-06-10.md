...
It defines safe audit fields, correlation IDs, actor/scope/result metadata, forbidden secret markers and required tests before any write API route or fake-runner implementation. `WAPI-I004` was then closed as operation status model; `WAPI-I003` was then closed as scoped write-token model; `WAPI-I002` was then closed as config delivery decoupling. Current next recommendation after `WAPI-I002` closure is `WAPI-I001` `/api/clients` design without live CRUD with `live_write_authorized: no`.
