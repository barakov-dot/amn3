...
| Public/self-service config delivery policy | `implemented-pushed-local-gate-complete` | `amn2` | branch `codex/public-config-delivery-policy-contract`, commit `2ef3af7`; evidence `research/amn2/public-self-service-config-delivery-policy-implementation.md`; focused `31 passed`, broader `78 passed`, full `577 passed` | Использовать как no-route share-token/policy baseline; public download, self-service download, API `config:read` и Local Agent `/configs` остаются отдельными gates |
...
5. Route/Auth machine-checkable binding tests выполнены и запушены в `amn2/codex/route-auth-binding-tests`, commit `f9d2c79`; slice добавляет `app/security/surface_bindings.py`, binding tests и фиксирует два policy drift случая без route expansion.
6. Backup/import dangerous API design подготовлен в `research/amn2/backup-import-dangerous-api-design.md`; пока VPS не готов, следующий local-only перенос - policy registry and restore-preview contract, без web/API full backup, restore apply или import apply.
7. Manager config export contract выполнен и запушен в `amn2/codex/manager-config-export-contract`, commit `4d4e7a4`; использовать как no-route typed export adapter baseline.
8. Public/self-service config delivery policy выполнен и запушен в `amn2/codex/public-config-delivery-policy-contract`, commit `2ef3af7`; использовать как no-route share-token/policy baseline.
