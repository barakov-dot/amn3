...
| API/Web panel finish slice | `plan-prepared-awaits-amn2-implementation` | AMN3 -> `amn2` | plan `docs/superpowers/plans/2026-06-04-amn2-api-web-panel-finish.md`; VPS runbook `docs/AMN2_API_WEB_PANEL_VPS_TEST_RUNBOOK.ru.md` | Следующий безопасный шаг: web-admin-only API readiness/status и API token lifecycle UI поверх verified read-only API baseline; VPS тестировать через loopback API smoke и SSH tunnel web check, без live apply |
...
2. Следующий безопасный API/web-panel шаг: реализовать web-admin-only `API readiness/status` и `API token lifecycle` UI по `docs/superpowers/plans/2026-06-04-amn2-api-web-panel-finish.md`.
...
4. VPS install/update package от production head `5f12736` собран и готов к загрузке на VPS; для будущего web/API теста использовать `docs/AMN2_API_WEB_PANEL_VPS_TEST_RUNBOOK.ru.md`.
