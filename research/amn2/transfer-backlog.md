...
Controlled prod access-path confirmation 2026-06-07: web/admin access is through an operator-approved HTTPS reverse proxy, and public API port `3040` is not exposed. Evidence: `research/amn2/controlled-prod-reverse-proxy-confirmation-2026-06-07.md`. Final controlled-prod decision remains pending until the operator returns `controlled-prod-ready`, `needs-fix` or `defer-prod`.
...
current git-head package status: read-only-vps-smoke-pass
...
controlled prod readiness: readiness-prefill-recorded, reverse-proxy-access-confirmed, final decision pending
