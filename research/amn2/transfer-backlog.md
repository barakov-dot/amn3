...
Phase 4 NG-X003 stale wording cleanup 2026-06-10: completed as AMN3 docs-only evidence `research/amn2/phase-4-ng-x003-stale-wording-cleanup-2026-06-10.md`. It removed stale active-next references after the simple-task closure and updated the visible next recommendation to `NG-X001` gate naming consistency. No AMN2 code, runtime route, implementation, route change, config delivery, live VPS command, public exposure or production mutation was added. Next recommended docs-only task is `NG-X001` gate naming consistency with `live_write_authorized: no`.
