...
| API/Web panel finish slice | `merged-in-stable-local-gate-complete-package-published` | `amn2` stable branch + AMN3 evidence | branch `codex/api-web-panel-finish`, commit `294803e`; fast-forward merged into `codex-vps-test-prep`; evidence `research/amn2/api-web-panel-finish-implementation.md`; plan `docs/superpowers/plans/2026-06-04-amn2-api-web-panel-finish.md`; VPS runbook `docs/AMN2_API_WEB_PANEL_VPS_TEST_RUNBOOK.ru.md`; package `dist/amn2-vps-update-and-smoke-kit-294803e.zip` | Run real VPS API/web-panel test via loopback + SSH tunnel, без live apply; route/API expansion remains closed |
...
2. API/web-panel finish slice реализован, fast-forward merged в stable `codex-vps-test-prep` at `294803e`; local full suite `594 passed`.
...
4. VPS install/update package от production head `294803e` опубликован; для web/API теста использовать `docs/AMN2_API_WEB_PANEL_VPS_TEST_RUNBOOK.ru.md`.
...
Remote operation VPS gate branch `codex/remote-operation-vps-gate-prep` была подготовлена поверх прежнего `codex-vps-test-prep` head `d0939d8`: dry-run metadata, Runtime Registry и SSH host key verifier baseline подтверждены локально, но real VPS verification еще не запускался. Перед отдельным remote-operation VPS gate ее нужно обновить/rebase поверх нового stable head `294803e`.
