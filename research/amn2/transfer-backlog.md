...
| Remote operation VPS gate candidate | `prepared-pushed-awaits-real-vps-gate` | `amn2` branch + AMN3 runbook | branch `codex/remote-operation-vps-gate-prep`, head `7281254`; runbook `research/amn2/vps-gate-remote-operation-dry-run-audit.md`; package `dist/amn2-remote-operation-vps-gate-7281254-update-kit.zip`, sha256 `85FE02C2D9F402562E36CD08990CCA0A891E9173D5257EFC52E5DDF8F5C2061B`; focused `71 passed`, full `603 passed` | Выполнить read-only/dry-run на VPS; single apply/revoke только после отдельного подтверждения |
...
Remote operation VPS gate branch `codex/remote-operation-vps-gate-prep` обновлена поверх stable head `294803e` и запушена как `7281254`: dry-run metadata, Runtime Registry, SSH host key verifier baseline и API/web-panel baseline подтверждены локально; real VPS verification еще не запускался.
