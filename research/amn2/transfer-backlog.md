...
| VPS install/update package | `published-updated-stable-5f12736` | AMN3 package for `amn2` | `dist/amn2-vps-install-5f12736.zip`, sha256 `41817BD8F5F6F7F4BE3DBDA53788645A02B2BAE1664A1FB5AF6B979512238A29`; install package includes `amn2_api_loopback_smoke.sh` and installer `Next steps` run API-only smoke with `AMN2_RUN_PREFLIGHT=0`; `dist/amn2-vps-update-and-smoke-kit-5f12736.zip`, sha256 `79BFFCA120D95FA4A68163CAD80F56C61E241035FDCD0FF6DC318BBA1E594A85`; historical `d0939d8` package remains available | Использовать `install` для чистой установки, `update+smoke` для существующего `/opt/amn2` с сохранением `.env`/`data`/`venv`/`servers.yml`; `server preflight` запускать только как отдельный SSH/server dry-run gate |
...
2. Не открывать повторный local-only API implementation slice: first API shell уже прошел local suite, real VPS loopback smoke и stable merge. Raw token/header/hash/config/keys/PSK не фиксировать в чатах.
3. VPS install/update package от production head `5f12736` собран и готов к загрузке на VPS.
