...
6. Secret inventory registry выполнен и запушен в `amn2/codex/secret-inventory-registry`, commit `9ce42f4`; использовать как machine-checkable secret baseline без route expansion.
7. Backup/import policy contract выполнен и запушен в `amn2/codex/backup-import-policy-contract`, head `afb2702` with foundation commit `d2c160b`; использовать как no-route policy/preview baseline, без web/API full backup, restore apply или import apply.
8. Manager config export contract выполнен и запушен в `amn2/codex/manager-config-export-contract`, commit `4d4e7a4`; использовать как no-route typed export adapter baseline.
9. Public/self-service config delivery policy выполнен и запушен в `amn2/codex/public-config-delivery-policy-contract`, commit `2ef3af7`; использовать как no-route share-token/policy baseline.
10. Docker manager safety contract уже зафиксирован в `research/amn2/docker-manager-design-note.md`; implementation plan писать только после VPS evidence.
11. Read-only clients/metrics endpoints рассматривать только после VPS evidence или отдельного route-exposure решения; privacy classification подготовлена в `research/amn2/read-only-metrics-privacy-classification.md`, Local Agent runtime metadata alignment - в `research/amn2/local-agent-runtime-metadata-alignment.md`, token lifecycle gate выполнен в `amn2/codex/api-token-lifecycle-gate`; stacked-вариант поверх route/auth binding доступен в `amn2/codex/api-token-lifecycle-gate-stacked`.
12. Domain exclusions и 2FA держать отложенными до закрытия текущих safety gates.
