...
- [Target server manual web/bot evidence 2026-06-09](target-server-manual-web-bot-evidence-2026-06-09.md) - new target VPS manual readiness pass: bot network check, web admin password/session secret presence, temporary loopback web `/login=200`, cleanup left `3030`/`3040` closed and peer count `0`; service-mode remains closed.
- [Next chat Phase 3 Service Mode Gate](../../docs/NEXT_CHAT_AMN2_PHASE_3_SERVICE_MODE.ru.md) - one-copy handoff for the next chat: controlled service-mode/prod-readiness gate after target VPS manual readiness.
...
5.4. New target VPS passed AWG2 runtime, live single disposable peer apply/revoke, and manual web/bot readiness gates; final peer count is `0`, direct public `3030`/`3040` remain closed, and service-mode/reverse-proxy cutover is still a separate gate.
