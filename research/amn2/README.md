...
- [Target server AWG2 runtime smoke evidence 2026-06-09](target-server-awg2-runtime-smoke-evidence-2026-06-09.md) - new target VPS `amnezia-awg2` Docker runtime pass: real `servers.yml` created on VPS, AMN2 loader accepted it, official read-only API smoke passed with `run_id=20260609T043158Z`, `checked_routes=6`; live peer writes remain closed.
- [Target server live peer gate evidence 2026-06-09](target-server-live-peer-gate-evidence-2026-06-09.md) - new target VPS verified-live gate for exactly one disposable test peer: dry-run apply/revoke, live apply/sync/revoke/sync, final peer count `0`, post-gate API smoke `run_id=20260609T045546Z`, `checked_routes=6`.
- [Target server manual web/bot evidence 2026-06-09](target-server-manual-web-bot-evidence-2026-06-09.md) - historical manual readiness pass before service-mode: bot network check, web admin password/session secret presence, temporary loopback web `/login=200`, cleanup left `3030`/`3040` closed and peer count `0`.
- [Next chat Phase 3 Service Mode Gate](../../docs/NEXT_CHAT_AMN2_PHASE_3_SERVICE_MODE.ru.md) - active handoff: service-mode web/bot is now enabled/active loopback-only, operator access is SSH tunnel only, public API/direct web/HTTPS/write gates remain closed.
- [Next chat Phase 4 Unified Product Gate](../../docs/NEXT_CHAT_AMN2_PHASE_4_UNIFIED_PRODUCT_GATE.ru.md) - active main-chat handoff after Phase 3: unified local/read-only product/API planning for AMN2, target VPS, PRVTPRO and KYORESUAS lines.
- [AMN Unified Prod Gate Handoff](../../docs/AMN_UNIFIED_PROD_GATE_HANDOFF.ru.md) - coordination packet for merging AMN2/API, Phase 3 service-mode target VPS evidence and PRVTPRO/Web Panel candidate work without mixing live commands.
...
- [Phase 4 unified product gate handoff 2026-06-09](phase-4-unified-product-gate-handoff-2026-06-09.md) - AMN3 research note recording the transition to Phase 4 local/read-only unified planning.
...
5.3. AMN3 package `f7f6131` passed read-only status-alignment source-overlay smoke; write/config/backup/agent/public API gates remain closed.
5.4. New target VPS passed AWG2 runtime, live peer gates, manual web/bot readiness and Phase 3 service-mode loopback gates. Current target state: web/bot active via systemd, web/admin loopback-only on `127.0.0.1:3030`, operator access SSH tunnel only, live peer count `2`, `Neobyatnaya-AMNZ-3/-4` revoked, public/direct `3030`, public API `3040`, TCP `80/443` and Caddy/HTTPS remain absent/deferred.
